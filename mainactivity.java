	...
	private void initialize(Bundle _savedInstanceState) {
		...
		parte.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (listasecciones.get((int)(_position)).equals("Parte")) {
					...
					transferencias.edit().putString("MÃ©todo", "1").commit();
					if (transferencias.getString("MÃ©todo", "").equals("2")) {
						if (transferencias.getString("SecciÃ³n", "").equals("Editar")) {
						...
						if (transferencias.getString("Parte", "").equals("Cuadro de ediciÃ³n")) {
		...
		tipoconfiguracion.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (listatipoconfiguracion.get((int)(_position)).equals("Dimensiones")) {
					...
					transferencias.edit().putString("Tipo configuraciÃ³n", listatipoconfiguracion.get((int)(_position))).commit();
		...
		abajo.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View _view) {
				transferencias.edit().putString("DirecciÃ³n", "Abajo").commit();
				...
				if (transferencias.getString("ConfiguraciÃ³n cambiada", "").equals("No")) {
					botonguardarconfiguracion.setVisibility(View.VISIBLE);
					transferencias.edit().putString("ConfiguraciÃ³n cambiada", "Si").commit();
				}
				else {
		...
		arriba.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View _view) {
				arriba.setOnLongClickListener(new View.OnLongClickListener() {
					@Override
					public boolean onLongClick(View _view) {
						transferencias.edit().putString("DirecciÃ³n", "Arriba").commit();
						if (contadormover < contadormaximo) {
							contadormover = contadormover * 2;
						}
						else {
		...
		button24.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				_ocultaropciones();
				if (transferencias.getString("SecciÃ³n", "").equals("Calendario")) {
					transferencias.edit().putString("Mostrar opciÃ³n", "Calendario").commit();
					_mostraropcion();
				}
				else {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (_charSeq.equals("")) {
					...
					if (_charSeq.equals("(No)")) {
						...
						aviso.setMessage("Nombre no vÃ¡lido");
						...
						transferencias.edit().putString("Ãšltimo nombre", _charSeq).commit();
						transferencias.edit().putString("MÃ©todo", "1").commit();
						if (transferencias.getString("MÃ©todo", "").equals("2")) {
							if (transferencias.getString("Ãšltimo nombre", "").equals(_charSeq)) {
		...
		botonguardar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				//inicio registro 
				_fechayhora();
				transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Inicio botÃ³n guardar archivo. ".concat(""))))).commit();
				if (registro.size() > 0) {
				...
				if (transferencias.getString("Guardar registro automÃ¡ticamente", "").equals("Si") || transferencias.getString("Registrar en archivo", "").equals("Si")) {
				...
				//fin de registro 
				transferencias.edit().putString("MÃ©todo", "1").commit();
				if (transferencias.getString("MÃ©todo", "").equals("1")) {
					transferencias.edit().putString("Continuar", "Si").commit();
					if (transferencias.getString("Aviso guardado", "").equals("No")) {
						aviso.setTitle("AtenciÃ³n ");
						aviso.setMessage("Los cambios en los crÃ­ticos de aviso no se han guardado. Si continÃºa, se perderÃ¡n los criterios. Â¿Continuar?. ".concat("".concat("".concat(""))));
						aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								transferencias.edit().putString("Continuar", "Si").commit();
					...
					if (transferencias.getString("Continuar", "").equals("Si")) {
						...
						//guardar datos
						if (listadia.size() > 0) {
							transferencias.edit().putString("Mostrar opciÃ³n", "Cargar").commit();
							_mostraropcion();
							contador = 0;
							...
							for(int _repeat1911 = 0; _repeat1911 < (int)(listadia.size()); _repeat1911++) {
								//poner cada lectura de matriz en una condiciÃ³n para evitar errores por falta de datos
								if (contador < listadia.size()) {
									transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(DÃ­a)".concat("\n"))).commit();
									...
									transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(AÃ±o)".concat("\n"))).commit();
							...
							transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(NotificaciÃ³n)\n".concat(transferencias.getString("NotificaciÃ³n", "").concat("\n(Ventana)\n".concat(transferencias.getString("Ventana", "").concat("\n(Decir)\n".concat(transferencias.getString("Decir", "").concat("\n")))))))).commit();
							...
							transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(Evento prÃ³ximo)\n".concat(transferencias.getString("Evento prÃ³ximo", "").concat("\n")))).commit();
							...
							transferencias.edit().putString("MÃ©todo", "1").commit();
							if (transferencias.getString("MÃ©todo", "").equals("2")) {
								//habilitar si no se usa el cÃ³digo proporcionado por el asistente 
							...
							transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". NÃºmero de eventos: ".concat(String.valueOf((long)(listaavisoseventospersonalizados.size())).concat("")))))).commit();
									...
									if (listaavisosformaspersonalizadas.size() > 0) {
										if (listaavisosfiltrospersonalizados.size() > 0) {
											if (listaavisosaudiopersonalizados.size() > 0) {
												//no es necesario leer el nÃºmero de eventos
												//prueba para guardar avisos mediante asistente 11/11/24
												
												// Inicializa el StringBuilder para construir los datos
												StringBuilder datos = new StringBuilder();
												
												// MantÃ©n los datos preexistentes sin aÃ±adir un salto de lÃ­nea adicional
												datos.append(transferencias.getString("Dato", ""));
												
												// AÃ±ade la categorÃ­a (Eventos) y el nÃºmero de elementos
												...
												// AÃ±ade la categorÃ­a (Momentos) y sus datos
												...
												// AÃ±ade la categorÃ­a (Formas) y sus datos
												datos.append("(Formas)\n");
												...
												// AÃ±ade la categorÃ­a (Filtros) y sus datos
												...
												// AÃ±ade la categorÃ­a (Audios) y sus datos
												datos.append("(Audios)\n");
...
// MantÃ©n los datos preexistentes
...
*/
							...
							if (transferencias.getString("MÃ©todo", "").equals("2")) {
								...
								listaavisoeventos.add("DÃ­a siguiente");
								...
								listaavisomomento.add("Al mostrar aplicaciÃ³n");
								...
								listaavisoformas.add("NotificaciÃ³n completa");
								listaavisoformas.add("Ventana emergente");
								listaavisosfiltros.add("Seleccione filtro/s de aviso");
								listaavisosfiltros.add("Solo dÃ­as de la semana");
								listaavisosfiltros.add("Excepto dÃ­as de la semana");
							}
							else {
						...
						setTitle(transferencias.getString("SecciÃ³n", "").concat(" (".concat(nombre.getText().toString().concat(") "))));
						SketchwareUtil.showMessage(getApplicationContext(), "Guardado");
						if (nombre.getText().toString().equals(transferencias.getString("Ãšltimo nombre", ""))) {
							
						}
						else {
							aviso.setTitle("Nota con texto duplicado");
							aviso.setMessage("Â¿Borrar nota anterior? (".concat(transferencias.getString("Ãšltimo nombre", "").concat(") ".concat(""))));
							aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface _dialog, int _which) {
									FileUtil.deleteFile(transferencias.getString("Ruta fichero", "").concat("/".concat(transferencias.getString("Ãšltimo nombre", "").concat(".txt"))));
									SketchwareUtil.showMessage(getApplicationContext(), "Cambiado nombre de nota".concat(""));
									transferencias.edit().putString("Ãšltimo nombre", transferencias.getString("Nombre nota", "")).commit();
								}
							});
							aviso.setNegativeButton("No", new DialogInterface.OnClickListener() {
								@Override
						...
						if (transferencias.getString("MÃ©todo", "").equals("2")) {
							aviso.setTitle("Nota (");
							aviso.setMessage("Nombre nota (".concat(nombre.getText().toString().concat("). Nombre anterior (".concat(transferencias.getString("Ãšltimo nombre", "").concat(") ".concat(""))))));
							aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface _dialog, int _which) {
				...
				//registro 
				_fechayhora();
				transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Fin de botÃ³n guardar archivo. ".concat(""))))).commit();
				if (registro.size() > 0) {
					registro.set((int)0, transferencias.getString("Registro", ""));
				}
				else {
			...
			public void onClick(View _view) {
				if (voz.isSpeaking()) {
					...
					if (texto.getText().toString().equals("")) {
						...
						if (transferencias.getString("Tipo de bÃºsqueda", "").equals("Texto")) {
		...
		masopcionesedicion.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (listaedicion.get((int)(_position)).equals("InformaciÃ³n de la nota")) {
					aviso.setTitle(listaedicion.get((int)(_position)).concat("".concat("".concat(""))));
					aviso.setMessage("LÃ­neas: ".concat(String.valueOf((long)(lineas.size())).concat("".concat(""))));
		...
		opcionesnotas.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				final int _position = _param3;
				transferencias.edit().putString("SecciÃ³n", listasecciones.get((int)(_position))).commit();
			}
		...
		button10.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				transferencias.edit().putString("SecciÃ³n opciones", "Buscar calendario").commit();
				_controlopciones();
			}
		});
		...
		registroizquierda.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if ((contadorbuscar - 1) > -1) {
					...
					transferencias.edit().putString("DÃ­a seleccionado", String.valueOf((long)(diaencontrado.get((int)(contadorbuscar)).doubleValue()))).commit();
					...
					transferencias.edit().putString("AÃ±o seleccionado", String.valueOf((long)(anoencontrado.get((int)(contadorbuscar)).doubleValue()))).commit();
					_comprobarfecha();
		...
		spinnerbusqueda.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("DÃ­a seleccionado", String.valueOf((long)(listadia.get((int)(diaencontrado.get((int)(_position)).doubleValue())).doubleValue()))).commit();
				...
				transferencias.edit().putString("AÃ±o seleccionado", String.valueOf((long)(listaano.get((int)(anoencontrado.get((int)(_position)).doubleValue())).doubleValue()))).commit();
				_comprobarfecha();
		...
		checkbox4.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					transferencias.edit().putString("Tipo de bÃºsqueda", "Lista").commit();
				}
				else {
					transferencias.edit().putString("Tipo de bÃºsqueda", "Texto").commit();
				}
				if (transferencias.getString("Tipo de bÃºsqueda", "").equals("Lista")) {
					if (transferencias.getString("SecciÃ³n", "").equals("Editar")) {
						transferencias.edit().putString("Mostrar secciÃ³n", "Buscar por lista").commit();
						_mostrarseccion();
						...
						if (transferencias.getString("Cambiado", "").equals("Si")) {
									...
									runOnUiThread(new Runnable() {
										...
										public void run() {
											if (contador < slist.size()) {
												...
												transferencias.edit().putString("Mostrar opciÃ³n", "Editar").commit();
												_mostraropcion();
							...
							transferencias.edit().putString("Cargando", "Si").commit();
							transferencias.edit().putString("Mostrar opciÃ³n", "Buscar").commit();
							_mostraropcion();
							buscarporlista.setAdapter(new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_list_item_1, listabuscar));
							((BaseAdapter)buscarporlista.getAdapter()).notifyDataSetChanged();
							textview8.setText("Buscar en lista. Elementos: ".concat(String.valueOf((long)(listabuscar.size()))));
					...
					if (transferencias.getString("SecciÃ³n", "").equals("Notas")) {
						transferencias.edit().putString("Mostrar secciÃ³n", "Lista de notas").commit();
						_mostrarseccion();
					}
					else {
		...
		edittext2.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				final String _charSeq = _param1.toString();
				find = _charSeq;
				if (transferencias.getString("Ãšltima bÃºsqueda", "").equals(_charSeq)) {
				...
				if (edittext2.getText().toString().equals("")) {
					...
					if (transferencias.getString("Tipo de bÃºsqueda", "").equals("Texto")) {
						...
						if (texto.getText().toString().equals("")) {
							...
							texto.requestFocus();
							//Buscar posiciÃ³n de palabra 
							if (contador == 0) {
								transferencias.edit().putString("LÃ­mite", String.valueOf((long)(caracteres + edittext2.getText().toString().length()))).commit();
								transferencias.edit().putString("Filtro", "").commit();
								transferencias.edit().putString("Texto", "").commit();
								transferencias.edit().putString("PosiciÃ³n encontrada", "0").commit();
								str = texto.getText().toString();
								slist1.clear();
								//calcular nÃºmero de caracteres. Restar 1
											...
											if (transferencias.getString("Buscando", "").equals("No")) {
												if (transferencias.getString("Fin", "").equals("No")) {
													if ((contador + edittext2.getText().toString().length()) < slist1.size()) {
														transferencias.edit().putString("LÃ­mite", String.valueOf((long)(contador + edittext2.getText().toString().length()))).commit();
														...
														if (edittext2.getText().toString().toLowerCase().equals(transferencias.getString("Filtro", "").toLowerCase())) {
															...
															transferencias.edit().putString("PosiciÃ³n encontrada", String.valueOf((long)(contador))).commit();
															transferencias.edit().putString("Fin", "Si").commit();
														...
														contador++;
														//fin de contador de caracteres aquÃ­ por probable fallo del compilador en else
														if ((contador + edittext2.getText().toString().length()) < slist1.size()) {
														...
														//fallo de contador. Nada entra aquÃ­ aunque el contador deje de ser menor que el criterio establecido 
													}
													//
												}
												else {
													if (veces < 1) {
														if (Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", "")) > 0) {
															...
															textocargar.setText("Fin de bÃºsqueda. No encontrado (".concat(String.valueOf((long)(contador + 1)).concat("/".concat(String.valueOf((long)(slist1.size())).concat(") (".concat(transferencias.getString("Filtro", "").concat("). Fin ".concat(transferencias.getString("Fin", "").concat(". Temporizador activo: ".concat(String.valueOf((long)(veces)).concat("")))))))))));
															encontrados.setText("Fin de bÃºsqueda. No encontrado (".concat(String.valueOf((long)(contador + 1)).concat("/".concat(String.valueOf((long)(slist1.size())).concat(") (".concat(transferencias.getString("Filtro", "").concat("). Fin ".concat(transferencias.getString("Fin", "").concat(". Temporizador activo: ".concat(String.valueOf((long)(veces)).concat("")))))))))));
														}
														//posiciÃ³n de cursor
														textoposicion.setText("");
														posicioncursor = 0;
														for(int _repeat4364 = 0; _repeat4364 < (int)(Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", ""))); _repeat4364++) {
															if (posicioncursor < Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", ""))) {
																posicioncursor++;
														...
														texto.requestFocus();
														((EditText)texto).setSelection((int)Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", "")), (int)Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", "")) + edittext2.getText().toString().length());
														textoposicion.getBackground().setColorFilter(0xFFF44336, PorterDuff.Mode.MULTIPLY);
														//fin
														//mover texto
														if (Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", "")) > (64 * 28)) {
															int deslizartextovar = (int) Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", "")) * 67;
															deslizartexto.scrollTo(0, deslizartextovar);
														}
														else {
														...
														if (transferencias.getString("Marcar", "").equals("Si")) {
															transferencias.edit().putString("Marcar", "No").commit();
															transferencias.edit().putString("Ãšltima bÃºsqueda", edittext2.getText().toString()).commit();
														}
														else {
							...
							//fin de bÃºsqueda de posiciÃ³n 
							transferencias.edit().putString("MÃ©todo", "1").commit();
							if (transferencias.getString("MÃ©todo", "").equals("2")) {
								if (transferencias.getString("Primera bÃºsqueda", "").equals("Si")) {
									transferencias.edit().putString("Primera bÃºsqueda", "No").commit();
									//primera bÃºsqueda
									//posiciÃ³n de texto
									textobuscar = edittext2.getText().toString();
									posicionx.clear();
									posicionx1.clear();
									//buscar la posiciÃ³n de todos los textos
									...
									try{
										...
										//colocar cursor. Si se usa matriz numÃ©rica, usar get at
						...
						contador1 = 0;
						transferencias.edit().putString("Fin", "No").commit();
						transferencias.edit().putString("PosiciÃ³n encontrada", "No").commit();
						transferencias.edit().putString("Buscando", "Si").commit();
								...
								runOnUiThread(new Runnable() {
									...
									public void run() {
												...
												for(int _repeat2206 = 0; _repeat2206 < (int)(slist1.size()); _repeat2206++) {
													if ((caracteres + edittext2.getText().toString().length()) < slist1.size()) {
														...
														if (edittext2.getText().toString().toLowerCase().equals(transferencias.getString("Filtro", "").toLowerCase())) {
															...
															transferencias.edit().putString("PosiciÃ³n encontrada", String.valueOf((long)(contador1))).commit();
															transferencias.edit().putString("Fin", "Si").commit();
														}
														else {
										...
										if (transferencias.getString("Fin", "").equals("Si")) {
											if (transferencias.getString("PosiciÃ³n encontrada", "").equals("No")) {
												textocargar.setText("Fin de bÃºsqueda. No encontrado (".concat(String.valueOf((long)(contador1 + 1)).concat("/".concat(String.valueOf((long)(listabuscar.size())).concat(") ".concat(edittext2.getText().toString().concat("")))))));
												encontrados.setText("Fin de bÃºsqueda. No encontrado (".concat(String.valueOf((long)(contador1 + 1)).concat("/".concat(String.valueOf((long)(listabuscar.size())).concat(") ".concat(edittext2.getText().toString().concat("")))))));
											}
											else {
												...
												buscarporlista.smoothScrollToPosition((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n encontrada", ""))));
											}
											temporizadorcargar.cancel();
										}
										else {
		...
		button34.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				transferencias.edit().putString("Mostrar opciÃ³n", "Organizar").commit();
				_mostraropcion();
			}
		});
		...
		botonguardarcalendario.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				transferencias.edit().putString("Cambiado1", "No").commit();
				transferencias.edit().putString("Ãšltimo dÃ­a", transferencias.getString("DÃ­a seleccionado", "")).commit();
				transferencias.edit().putString("Ãšltimo mes", transferencias.getString("Mes seleccionado", "")).commit();
				transferencias.edit().putString("Ãšltimo aÃ±o", transferencias.getString("AÃ±o seleccionado", "")).commit();
				transferencias.edit().putString("Ãšltimo estado", transferencias.getString("Estado", "")).commit();
				transferencias.edit().putString("Ãšltimo hora desde", transferencias.getString("Hora desde", "")).commit();
				transferencias.edit().putString("Ãšltimo hora hasta", transferencias.getString("Hora hasta", "")).commit();
				transferencias.edit().putString("Ãšltimo comentario", transferencias.getString("Comentario", "")).commit();
				botonguardar.setVisibility(View.VISIBLE);
				contador = 0;
				transferencias.edit().putString("MÃ©todo", "1").commit();
				if (transferencias.getString("MÃ©todo", "").equals("2")) {
					//completar matriz si no tiene los mismos datos que los demÃ¡s campos 
					...
					for(int _repeat19 = 0; _repeat19 < (int)((listadia.size() - 1)); _repeat19++) {
						if (listadia.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))) {
							if (listames.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes seleccionado", ""))) {
								if (listaano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))) {
				...
				if (transferencias.getString("Encontrado", "").equals("Si")) {
					aviso.setTitle("Fecha existente (".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat(") ".concat("".concat("".concat(""))))))))));
					aviso.setMessage("Â¿Sustituir? ".concat("".concat("".concat(""))));
					aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							listadia.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("DÃ­a seleccionado", "")));
							listames.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Mes seleccionado", "")));
							listaano.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("AÃ±o seleccionado", "")));
							listaestado.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Estado", ""));
							comentarios.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Comentario", ""));
							horadesde.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Hora desde", "")));
							horahasta.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Hora hasta", "")));
							horarioseleccionado.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Horario seleccionado", ""));
							...
							registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Sustituido. DÃ­a de la semana: ".concat(transferencias.getString("DÃ­a de la semana seleccionado", "").concat(". Fecha seleccionada: ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat(". Fecha encontrada: ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat(". Comentario: ".concat(transferencias.getString("Comentario", "").concat(". PosiciÃ³n: ".concat(transferencias.getString("PosiciÃ³n", "").concat("/".concat(String.valueOf((long)(listadia.size())).concat(""))))))))))))))))))))))));
					...
					//guardar evento
					listadia.add(Double.valueOf(Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))));
					listames.add(Double.valueOf(Double.parseDouble(transferencias.getString("Mes seleccionado", ""))));
					listaano.add(Double.valueOf(Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))));
		...
		botonbuscar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (listadia.size() > 0) {
					...
					transferencias.edit().putString("Criterio de bÃºsqueda", "No").commit();
					...
					if (transferencias.getString("Comentario", "").equals("")) {
						...
						transferencias.edit().putString("Criterio de bÃºsqueda", "Si").commit();
					...
					for(int _repeat293 = 0; _repeat293 < (int)(listadia.size()); _repeat293++) {
						if (contadorbuscar < listadia.size()) {
							...
							if (transferencias.getString("Criterio de bÃºsqueda", "").equals("Si")) {
								if (transferencias.getString("Filtro comentario", "").equals("Si")) {
									if (comentarios.get((int)(contadorbuscar)).contains(transferencias.getString("Comentario", ""))) {
										transferencias.edit().putString("Comentario encontrado", "Si").commit();
									}
								...
								if (transferencias.getString("Encontrado", "").contains("Si")) {
									...
									if ((listadia.get((int)(contadorbuscar)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))) && ((listames.get((int)(contadorbuscar)).doubleValue() == Double.parseDouble(transferencias.getString("Mes seleccionado", ""))) && (listaano.get((int)(contadorbuscar)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))))) {
					...
					registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Buscando. Criterios: ".concat(transferencias.getString("Criterio de bÃºsqueda", "").concat(". Criterio comentario: ".concat(transferencias.getString("Filtro comentario", "").concat(". Fechas encontradas: ".concat(String.valueOf((long)(fechasencontradas.size())).concat("".concat("".concat(""))))))))))));
					if (diaencontrado.size() > 0) {
						...
						registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". BÃºsqueda. Encontrados: ".concat(String.valueOf((long)(diaencontrado.size())).concat(". Criterios de bÃºsqueda: ".concat(transferencias.getString("Criterios de bÃºsqueda", "").concat("".concat("")))))))));
						...
						for(int _repeat1169 = 0; _repeat1169 < (int)(diaencontrado.size()); _repeat1169++) {
							if (contador1 < diaencontrado.size()) {
								if ((anoencontrado.get((int)(contador1)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o", ""))) || (0 > 0)) {
		...
		button52.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Copiado en memoria registro de fecha: ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat("".concat("".concat(""))))))))))))));
		...
		button53.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				for(int _repeat60 = 0; _repeat60 < (int)(listadia.size()); _repeat60++) {
					if (contador < listadia.size()) {
						if ((listadia.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))) && ((listames.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes seleccionado", ""))) && (listaano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))))) {
							transferencias.edit().putString("PosiciÃ³n", String.valueOf((long)(contador))).commit();
							transferencias.edit().putString("Encontrado", "Si").commit();
							break;
						}
						else {
				...
				if (transferencias.getString("Encontrado", "").equals("Si")) {
					aviso.setTitle("Â¿Sustituir este registro con estos datos? ");
					...
					aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface _dialog, int _which) {
							...
							listames.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Mes seleccionado", "")));
							listaano.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("AÃ±o seleccionado", "")));
							listaestado.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Copia estado", ""));
							comentarios.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Copia comentario", ""));
							horadesde.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Copia hora desde", "")));
							horahasta.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), Double.parseDouble(transferencias.getString("Copia hora hasta", "")));
							horarioseleccionado.set((int)Double.parseDouble(transferencias.getString("PosiciÃ³n", "")), transferencias.getString("Copia horario seleccionado", ""));
							registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Copiar. Sustituido registro ".concat(String.valueOf((long)(listadia.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(" por ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat("".concat("".concat("".concat("")))))))))))))))))));
						}
					});
					aviso.setNegativeButton("No", new DialogInterface.OnClickListener() {
						@Override
					...
					registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Copiar. Copiado registro ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat("".concat("".concat("".concat("")))))))))))));
				}
				temporizador = new TimerTask() {
					...
					public void run() {
						runOnUiThread(new Runnable() {
							...
							public void run() {
								_comprobarfecha();
								registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" Comprobar nuevos datos de fecha ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat("".concat("".concat("".concat("")))))))))))));
								_actualizarregistro();
								temporizador.cancel();
							}
						});
		...
		borrarevento.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (transferencias.getString("BÃºsqueda", "").equals("Si")) {
					transferencias.edit().putString("BÃºsqueda", "No").commit();
					registroizquierda.setVisibility(View.GONE);
					registroderecha.setVisibility(View.GONE);
					...
					if (transferencias.getString("Encontrado", "").equals("Si")) {
						aviso.setTitle("Â¿Borrar ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("? ")))))));
						aviso.setMessage("".concat("".concat("".concat(""))));
						aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								listadia.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								listames.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								listaano.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								listaestado.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								comentarios.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								horadesde.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								horahasta.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))));
								...
								if (fechasencontradas.size() > 0) {
									...
									for(int _repeat51 = 0; _repeat51 < (int)(fechasencontradas.size()); _repeat51++) {
										if (contador < fechasencontradas.size()) {
											if (String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())))))).equals(fechasencontradas.get((int)(contador)))) {
		...
		button63.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				aviso.setTitle("Â¿EstÃ¡ segur@ de que quiere borrar todos los eventos?. ");
				aviso.setMessage("Este proceso tambiÃ©n borrarÃ¡ los criterios de aviso y no serÃ¡ permanente hasta que guarde el archivo. Si desea restaurar los datos, reinicie la aplicaciÃ³n. ".concat("".concat("".concat(""))));
				aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface _dialog, int _which) {
						_fechayhora();
						...
						transferencias.edit().putString("Dato", "Todos los eventos borrados por el usuario el ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" a las ".concat(transferencias.getString("Hora", "").concat(":".concat(transferencias.getString("Minuto", "").concat(":".concat(transferencias.getString("Segundo", "").concat("\n(Fin)"))))))))))))).commit();
				...
				temporizador = new TimerTask() {
					...
					public void run() {
						runOnUiThread(new Runnable() {
							@Override
							public void run() {
								String diaTexto = transferencias.getString("DÃ­a", "0");
								String mesTexto = transferencias.getString("Mes", "0");
								String anoTexto = transferencias.getString("AÃ±o", "0");
		...
		button62.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				//teÃ³ricamente reduce el nombre del archivo si es demasiado largo 
				
				// Longitud mÃ¡xima permitida para el nombre base
				...
				String nuevoNombre = nombreBaseTruncado + "-" +
				        transferencias.getString("AÃ±o", "") + // Proporciona una cadena vacÃ­a como valor predeterminado si "AÃ±o" no existe
				        transferencias.getString("Mes", "") +  // Proporciona una cadena vacÃ­a como valor predeterminado si "Mes" no existe
				        transferencias.getString("DÃ­a", "") +  // Proporciona una cadena vacÃ­a como valor predeterminado si "DÃ­a" no existe
				        "_" +
				        transferencias.getString("Hora", "") +  // Proporciona una cadena vacÃ­a como valor predeterminado si "Hora" no existe
				        transferencias.getString("Minuto", "") + // Proporciona una cadena vacÃ­a como valor predeterminado si "Minuto" no existe
				        transferencias.getString("Segundo", "") + ".txt"; // Proporciona una cadena vacÃ­a como valor predeterminado si "Segundo" no existe
				
				
				_fechayhora();
				FileUtil.copyFile(transferencias.getString("Ruta fichero", "").concat("/".concat(nombre.getText().toString().concat(".txt"))), transferencias.getString("Ruta copias", "").concat("/".concat(nombre.getText().toString().concat("-".concat(transferencias.getString("Hora", "").concat("_".concat(transferencias.getString("Minuto", "").concat("_".concat(transferencias.getString("Segundo", "").concat(" ".concat(transferencias.getString("DÃ­a", "").concat("_".concat(transferencias.getString("Mes", "").concat("_".concat(transferencias.getString("AÃ±o", "").concat(".txt"))))))))))))))));
				if (FileUtil.isExistFile(transferencias.getString("Ruta copias", "").concat("/".concat(nombre.getText().toString().concat("-".concat(transferencias.getString("Hora", "").concat("_".concat(transferencias.getString("Minuto", "").concat("_".concat(transferencias.getString("Segundo", "").concat(" ".concat(transferencias.getString("DÃ­a", "").concat("_".concat(transferencias.getString("Mes", "").concat("_".concat(transferencias.getString("AÃ±o", "").concat(".txt"))))))))))))))))) {
					SketchwareUtil.showMessage(getApplicationContext(), "Copiado el archivo (".concat(transferencias.getString("Ruta fichero", "").concat("/".concat(nombre.getText().toString().concat(".txt) en la carpeta (".concat(transferencias.getString("Ruta copias", "").concat("/".concat(nombre.getText().toString().concat("-".concat(transferencias.getString("Hora", "").concat("_".concat(transferencias.getString("Minuto", "").concat("_".concat(transferencias.getString("Segundo", "").concat(" ".concat(transferencias.getString("DÃ­a", "").concat("_".concat(transferencias.getString("Mes", "").concat("_".concat(transferencias.getString("AÃ±o", "").concat(".txt")))))))))))))))))))));
				}
				else {
		...
		seccioncalendario.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "SecciÃ³n calendario ");
			}
		});
		...
		configuracion.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "ConfiguraciÃ³n ");
			}
		});
		...
		cuadroeditar.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "EdiciÃ³n");
			}
		});
		...
		general.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "SecciÃ³n 2");
			}
		});
		...
		alarma.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "SecciÃ³n 3");
			}
		});
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				if (nombrenota.equals("No hay notas")) {
					...
					transferencias.edit().putString("SecciÃ³n", "Editar").commit();
		...
		datepicker2.init(_calendar.get(Calendar.YEAR), _calendar.get(Calendar.MONTH), _calendar.get(Calendar.DAY_OF_MONTH), new DatePicker.OnDateChangedListener() {
			...
			public void onDateChanged(DatePicker _datePicker, int _year, int _month, int _day) {
				transferencias.edit().putString("DÃ­a seleccionado", String.valueOf((long)(_day))).commit();
				transferencias.edit().putString("Mes seleccionado", String.valueOf((long)(_month + 1))).commit();
				transferencias.edit().putString("AÃ±o seleccionado", String.valueOf((long)(_year))).commit();
				try {
						String input_date=transferencias.getString("AÃ±o", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("DÃ­a", ""))))); 
					...
					transferencias.edit().putString("DÃ­a de la semana seleccionado", finalDay).commit();
				...
				diasemana--;
				transferencias.edit().putString("DÃ­a de la semana seleccionado", String.valueOf((long)(diasemana))).commit();
				//Cambiar resto de datos si se cambia de fecha
				//fin 
				if (transferencias.getString("Ãšltimo dÃ­a", "").equals(transferencias.getString("DÃ­a seccionado", ""))) {
				...
				registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Fecha seleccionada ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat(". DÃ­as del mes: ".concat(String.valueOf((long)(diasmes)).concat("".concat("".concat("".concat("".concat("".concat("".concat("".concat("".concat("".concat("")))))))))))))))))))));
				if (transferencias.getString("Fecha", "").equals("Comprobar")) {
					if (transferencias.getString("Cambiado1", "").equals("Si")) {
						...
						aviso.setMessage("Si comprueba la fecha seleccionada se perderÃ¡n los datos modificados. Â¿Continuar? ".concat("".concat("".concat(""))));
						aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
								transferencias.edit().putString("Cambiado1", "No").commit();
				...
				if (transferencias.getString("Fecha", "").equals("Seleccionar dÃ­as del mes")) {
					//hay que usar al menos dos matrices porque si se usa solo una da error o la matriz dÃ­a guarda mal los datos 
					diasseleccionados.add(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))));
					if (transferencias.getString("Seleccionar", "").equals("Excepto dÃ­as")) {
						diaexcepcional.add(Double.valueOf(Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))));
						mesexcepcional.add(Double.valueOf(Double.parseDouble(transferencias.getString("Mes seleccionado", ""))));
						anoexcepcional.add(Double.valueOf(Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))));
						if (textoexceptodiasdelmes.getText().toString().equals("")) {
							textoexceptodiasdelmes.setText(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))));
						}
						else {
							textoexceptodiasdelmes.setText(textoexceptodiasdelmes.getText().toString().concat(", ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))))));
						}
					}
					else {
						
					}
					if (transferencias.getString("Seleccionar", "").equals("Solo dÃ­as")) {
						listasolodia.add(Double.valueOf(Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))));
						listasolomes.add(Double.valueOf(Double.parseDouble(transferencias.getString("Mes seleccionado", ""))));
						listasoloano.add(Double.valueOf(Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))));
						if (textoexceptodiasdelmes.getText().toString().equals("")) {
							textoexceptodiasdelmes.setText(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))));
						}
						else {
					...
					if (transferencias.getString("Seleccionar", "").equals("Solo dÃ­as para aviso")) {
						registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Seleccionar dÃ­as solo para aviso".concat("".concat("".concat("")))))));
						avisodia.add(Double.valueOf(Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))));
						avisomes.add(Double.valueOf(Double.parseDouble(transferencias.getString("Mes seleccionado", ""))));
						avisoano.add(Double.valueOf(Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))));
						if (avisoevento.size() > 0) {
							contador1 = 0;
							for(int _repeat1899 = 0; _repeat1899 < (int)(avisoevento.size()); _repeat1899++) {
								if (contador1 < avisoevento.size()) {
									if (transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))).equals(avisoevento.get((int)(contador1)))) {
										break;
									}
									else {
							...
							if (contador1 == avisoevento.size()) {
								avisoevento.add(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))));
								...
								if (transferencias.getString("MÃ©todo", "").equals("2")) {
									if (textofechasaviso.getText().toString().equals("")) {
										textofechasaviso.setText(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))));
									}
									else {
										textofechasaviso.setText(textofechasaviso.getText().toString().concat(", ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", ""))))))));
									}
								}
								else {
								...
								aviso.setTitle("Fecha (".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat(") repetida".concat(""))))))));
								aviso.setMessage("".concat("".concat("".concat(""))));
								aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
									@Override
									public void onClick(DialogInterface _dialog, int _which) {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (_charSeq.equals("")) {
					...
					if (transferencias.getString("Enviar texto", "").equals("Copiar desde fecha")) {
						...
						transferencias.edit().putString("Copiar dÃ­a desde", slist.get((int)(0))).commit();
						transferencias.edit().putString("Copiar mes desde", slist.get((int)(1))).commit();
						transferencias.edit().putString("Copiar aÃ±o desde", slist.get((int)(2))).commit();
					...
					if (transferencias.getString("Enviar texto", "").equals("Copiar hasta fecha")) {
						...
						_fechayhora();
						if (slist.size() > 2) {
							transferencias.edit().putString("Copiar dÃ­a hasta", slist.get((int)(0))).commit();
							transferencias.edit().putString("Copiar mes hasta", slist.get((int)(1))).commit();
							transferencias.edit().putString("Copiar aÃ±o hasta", slist.get((int)(2))).commit();
							...
							aviso.setMessage("Desde ".concat(transferencias.getString("Copiar dÃ­a desde", "").concat("/".concat(transferencias.getString("Copiar mes desde", "").concat("/".concat(transferencias.getString("Copiar aÃ±o desde", "").concat("\nhasta ".concat(slist.get((int)(0)).concat("/".concat(slist.get((int)(1)).concat("/".concat(slist.get((int)(2)).concat("".concat(""))))))))))))));
							aviso.setPositiveButton("Si", new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface _dialog, int _which) {
									...
									transferencias.edit().putString("DÃ­as", "0").commit();
									transferencias.edit().putString("DÃ­a", transferencias.getString("Copiar dÃ­a desde", "")).commit();
									transferencias.edit().putString("Mes", transferencias.getString("Copiar mes desde", "")).commit();
									transferencias.edit().putString("AÃ±o", transferencias.getString("Copiar aÃ±o desde", "")).commit();
									transferencias.edit().putString("Fechas", "0").commit();
									transferencias.edit().putString("VÃ¡lido", "No").commit();
									transferencias.edit().putString("DÃ­a de la semana", new SimpleDateFormat("EEEE").format(calendario.getTime())).commit();
									//Comprobar si la fecha de origen menor que la de destino
									if (Double.parseDouble(transferencias.getString("Copiar aÃ±o desde", "")) == Double.parseDouble(transferencias.getString("Copiar aÃ±o hasta", ""))) {
									...
									temporizadorcargar = new TimerTask() {
										...
										public void run() {
											runOnUiThread(new Runnable() {
												...
												public void run() {
													if (continuar) {
														transferencias.edit().putString("Fechas procesadas", String.valueOf((long)(Double.parseDouble(transferencias.getString("DÃ­as procesados", "")) + 1))).commit();
														//calcular mes 
														...
														diasmes = calendario.getActualMaximum(Calendar.DATE);
														transferencias.edit().putString("DÃ­as", String.valueOf((long)(diasmes))).commit();
														registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" Procesando fecha (".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(") ".concat(". Hasta fecha (".concat(transferencias.getString("Copiar dÃ­a hasta", "").concat("/".concat(transferencias.getString("Copiar mes hasta", "").concat("/".concat(transferencias.getString("Copiar aÃ±o hasta", "").concat("). ".concat(". Fechas procesadas: ".concat(transferencias.getString("Fechas procesadas", "").concat(". Fechas guardadas: ".concat(transferencias.getString("Fechas guardadas", "").concat(""))))))))))))))))))))));
														textocargar.setText(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" Procesando fecha (".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(") ".concat(". Hasta fecha (".concat(transferencias.getString("Copiar dÃ­a hasta", "").concat("/".concat(transferencias.getString("Copiar mes hasta", "").concat("/".concat(transferencias.getString("Copiar aÃ±o hasta", "").concat("). ".concat(". Fechas procesadas: ".concat(transferencias.getString("Fechas procesadas", "").concat(". Fechas guardadas: ".concat(transferencias.getString("Fechas guardadas", "").concat(""))))))))))))))))))))));
														transferencias.edit().putString("MÃ©todo", "1").commit();
														if (transferencias.getString("MÃ©todo", "").equals("2")) {
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 1) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Enero", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 2) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Febrero", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 3) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Marzo", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 4) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Abril", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 5) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Mayo", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 6) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Junio", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 7) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Julio", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 8) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Agosto", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 9) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Septiembre", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 10) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Octubre", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 11) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Noviembre", "")).commit();
															}
															else {
																
															}
															if (Double.parseDouble(transferencias.getString("Mes", "")) == 12) {
																transferencias.edit().putString("DÃ­as", transferencias.getString("Diciembre", "")).commit();
															}
															else {
														...
														for(int _repeat262 = 0; _repeat262 < (int)(listadia.size()); _repeat262++) {
															if (contador < listadia.size()) {
																if (Double.parseDouble(transferencias.getString("DÃ­a", "")) == listadia.get((int)(contador)).doubleValue()) {
																	if (Double.parseDouble(transferencias.getString("Mes", "")) == listames.get((int)(contador)).doubleValue()) {
																		if (Double.parseDouble(transferencias.getString("AÃ±o", "")) == listaano.get((int)(contador)).doubleValue()) {
																			transferencias.edit().putString("Encontrado", "Si").commit();
																			break;
																		}
																		else {
														...
														if (transferencias.getString("Filtro", "").equals("Si")) {
															...
															if (solodiassemana.isChecked()) {
																//Obtener y organizar dÃ­a de la semana 
																transferencias.edit().putString("Criterio dÃ­a de la semana", "No").commit();
																operardia = Double.parseDouble(transferencias.getString("DÃ­a", ""));
																operarmes = Double.parseDouble(transferencias.getString("Mes", ""));
																operarano = Double.parseDouble(transferencias.getString("AÃ±o", ""));
																...
																if (diasemana == 7) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "miÃ©rcoles").commit();
																}
																else {
																	
																}
																if (diasemana == 1) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "jueves").commit();
																}
																else {
																	
																}
																if (diasemana == 2) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "viernes").commit();
																}
																else {
																	
																}
																if (diasemana == 3) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "sÃ¡bado").commit();
																}
																else {
																	
																}
																if (diasemana == 4) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "domingo").commit();
																}
																else {
																	
																}
																if (diasemana == 5) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "lunes").commit();
																}
																else {
																	
																}
																if (diasemana == 6) {
																	transferencias.edit().putString("DÃ­a de la semana encontrado", "martes").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio lunes", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio martes", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio miÃ©rcoles", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio jueves", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio viernes", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio sÃ¡bado", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
																	
																}
																if (transferencias.getString("Criterio domingo", "").equals(transferencias.getString("DÃ­a de la semana encontrado", ""))) {
																	transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
																}
																else {
															...
															if (diasdelmesexcepcionales.isChecked()) {
																if (diaexcepcional.size() > 0) {
																	transferencias.edit().putString("MÃ©todo", "1").commit();
																	if (transferencias.getString("MÃ©todo", "").equals("1")) {
																		//buscar dÃ­a excepcional 
																		transferencias.edit().putString("DÃ­a excepcional", "No").commit();
																		contador = 0;
																		for(int _repeat1424 = 0; _repeat1424 < (int)(diaexcepcional.size()); _repeat1424++) {
																			if (contador < diaexcepcional.size()) {
																				if (diaexcepcional.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a", ""))) {
																					if (mesexcepcional.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes", ""))) {
																						if (anoexcepcional.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o", ""))) {
																							transferencias.edit().putString("DÃ­a excepcional", "Si").commit();
																							registro.add(transferencias.getString("Fecha completa", "").concat(". Fecha excepcional seleccionada ".concat(String.valueOf((long)(diaexcepcional.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(mesexcepcional.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(anoexcepcional.get((int)(contador)).doubleValue())).concat(" = ".concat("Fecha calculada ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" Encontrado ".concat(transferencias.getString("DÃ­a excepcional", "").concat(" (".concat(String.valueOf((long)(contador)).concat("/".concat(String.valueOf((long)(diaexcepcional.size())).concat(") ".concat(""))))))))))))))))))))));
																							break;
																						}
																						else {
															...
															if (solodias.isChecked()) {
																if (listasolodia.size() > 0) {
																	transferencias.edit().putString("MÃ©todo", "1").commit();
																	if (transferencias.getString("MÃ©todo", "").equals("1")) {
																		//buscar solo dÃ­as seleccionados 
																		transferencias.edit().putString("Solo dÃ­as", "No").commit();
																		contador = 0;
																		for(int _repeat3084 = 0; _repeat3084 < (int)(listasolodia.size()); _repeat3084++) {
																			if (contador < listasolodia.size()) {
																				if (listasolodia.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a", ""))) {
																					if (listasolomes.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes", ""))) {
																						if (listasoloano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o", ""))) {
																							transferencias.edit().putString("Solo dÃ­as", "Si").commit();
																							registro.add(transferencias.getString("Fecha completa", "").concat(". DÃ­a exclusivo ".concat(String.valueOf((long)(listasolodia.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listasolomes.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listasoloano.get((int)(contador)).doubleValue())).concat(" = ".concat("Fecha calculada ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" Encontrado ".concat(transferencias.getString("Solo dÃ­a ", "").concat(" (".concat(String.valueOf((long)(contador)).concat("/".concat(String.valueOf((long)(listasolodia.size())).concat(") ".concat(""))))))))))))))))))))));
																							break;
																						}
																						else {
															...
															if (transferencias.getString("DÃ­a excepcional", "").equals("Si")) {
																transferencias.edit().putString("Copiar", "No").commit();
															}
															else {
																transferencias.edit().putString("Copiar", "Si").commit();
																if (transferencias.getString("Criterio dÃ­a de la semana", "").equals("Si")) {
																	transferencias.edit().putString("Copiar", "Si").commit();
																}
																else {
																	transferencias.edit().putString("Copiar", "No").commit();
															...
															if (transferencias.getString("Solo dÃ­as", "").equals("Si")) {
																transferencias.edit().putString("Copiar", "Si").commit();
															}
															else {
															...
															if (transferencias.getString("Copiar", "").equals("Si")) {
																...
																if (transferencias.getString("Encontrado", "").equals("Si")) {
																	if (transferencias.getString("Sustituir", "").equals("Si")) {
																		listadia.set((int)contador, Double.parseDouble(transferencias.getString("DÃ­a", "")));
																		listames.set((int)contador, Double.parseDouble(transferencias.getString("Mes", "")));
																		listaano.set((int)contador, Double.parseDouble(transferencias.getString("AÃ±o", "")));
																		...
																		registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Sustituido registro ".concat(String.valueOf((long)(listadia.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(" por ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" sin filtro".concat("".concat("".concat("")))))))))))))))))));
																	}
																	else {
																	...
																	listadia.add(Double.valueOf(Double.parseDouble(transferencias.getString("DÃ­a", ""))));
																	listames.add(Double.valueOf(Double.parseDouble(transferencias.getString("Mes", ""))));
																	listaano.add(Double.valueOf(Double.parseDouble(transferencias.getString("AÃ±o", ""))));
																	listaestado.add(transferencias.getString("Estado", ""));
																	comentarios.add(transferencias.getString("Comentario", ""));
																	horadesde.add(Double.valueOf(Double.parseDouble(transferencias.getString("Hora desde", ""))));
																	horahasta.add(Double.valueOf(Double.parseDouble(transferencias.getString("Hora hasta", ""))));
																	horarioseleccionado.add(transferencias.getString("Horario seleccionado", ""));
																	textocargar.setText(textocargar.getText().toString().concat(". Copiado registro en fecha ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat("".concat("".concat(" ".concat("".concat("".concat("".concat("".concat("".concat("".concat("".concat(""))))))))))))))))));
																	registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Copiado registro ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat("en fecha/s ".concat("".concat("".concat("")))))))))))));
																	transferencias.edit().putString("Copiar fecha", "Si").commit();
																	//manejar fecha 
																}
																registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" La fecha ".concat(String.valueOf((long)(listadia.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(" - DÃ­a de la semana: ".concat(transferencias.getString("Criterio dÃ­a de la semana", "").concat(" = ".concat("criterio obtenido ".concat(transferencias.getString("DÃ­a de la semana encontrado", "").concat("/".concat(String.valueOf((long)(diasemana)).concat(". DÃ­a excepcional ".concat(". ".concat(transferencias.getString("DÃ­a excepcional", "").concat(". Copiar ".concat(transferencias.getString("Copiar", "").concat("".concat("")))))))))))))))))))))));
															}
															else {
																registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" La fecha (".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(") no cumple los criterios ".concat(" Criterio dÃ­a de la semana: ".concat(transferencias.getString("Criterio dÃ­a de la semana", "").concat(" = ".concat(transferencias.getString("DÃ­a de la semana encontrado", "").concat("/".concat(String.valueOf((long)(diasemana)).concat("")))))))))))))))));
															}
															transferencias.edit().putString("MÃ©todo", "1").commit();
															if (transferencias.getString("MÃ©todo", "").equals("2")) {
														...
														if ((Double.parseDouble(transferencias.getString("DÃ­a", "")) == Double.parseDouble(transferencias.getString("Copiar dÃ­a hasta", ""))) && ((Double.parseDouble(transferencias.getString("Mes", "")) == Double.parseDouble(transferencias.getString("Copiar mes hasta", ""))) && (Double.parseDouble(transferencias.getString("AÃ±o", "")) == Double.parseDouble(transferencias.getString("Copiar aÃ±o hasta", ""))))) {
															continuar = false;
															SketchwareUtil.showMessage(getApplicationContext(), "Copiado");
															registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" Fin fecha (".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(") ".concat(". Hasta fecha (".concat(transferencias.getString("Copiar dÃ­a hasta", "").concat("/".concat(transferencias.getString("Copiar mes hasta", "").concat("/".concat(transferencias.getString("Copiar aÃ±o hasta", "").concat("). ".concat(". Fechas procesadas: ".concat(transferencias.getString("Fechas procesadas", "").concat(". Fechas guardadas: ".concat(transferencias.getString("Fechas guardadas", "").concat(""))))))))))))))))))))));
														...
														transferencias.edit().putString("DÃ­a", String.valueOf((long)(Double.parseDouble(transferencias.getString("DÃ­a", "")) + 1))).commit();
														if (Double.parseDouble(transferencias.getString("DÃ­a", "")) > Double.parseDouble(transferencias.getString("DÃ­as", ""))) {
															transferencias.edit().putString("DÃ­a", "1").commit();
															if (Double.parseDouble(transferencias.getString("Mes", "")) < 12) {
																transferencias.edit().putString("Mes", String.valueOf((long)(Double.parseDouble(transferencias.getString("Mes", "")) + 1))).commit();
															}
															else {
																transferencias.edit().putString("AÃ±o", String.valueOf((long)(Double.parseDouble(transferencias.getString("AÃ±o", "")) + 1))).commit();
		...
		eventoproximo.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (transferencias.getString("Cargando", "").equals("No")) {
					if (_isChecked) {
						transferencias.edit().putString("Evento prÃ³ximo", "Si").commit();
						//Comprobar si el evento estÃ¡ repetido 
						contador1 = -1;
						if (avisoevento.size() > 0) {
							contador1 = 0;
							for(int _repeat20 = 0; _repeat20 < (int)(avisoevento.size()); _repeat20++) {
								if (contador1 < avisoevento.size()) {
									if (avisoevento.get((int)(contador1)).equals("Evento diario y prÃ³ximo dÃ­a")) {
										break;
									}
									else {
							...
							if (contador1 == avisoevento.size()) {
								avisoevento.add("Evento diario y prÃ³ximo dÃ­a");
								spinneravisoeventos.setAdapter(new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_spinner_dropdown_item, avisoevento));
								((ArrayAdapter)spinneravisoeventos.getAdapter()).notifyDataSetChanged();
								spinneravisoeventos.setSelection((int)(avisoevento.size() - 1));
								if (cuadroeventosaviso.getVisibility() == View.GONE) {
						...
						transferencias.edit().putString("Evento prÃ³ximo", "No").commit();
						cuadrocriteriosgenerales.setVisibility(View.VISIBLE);
					}
					registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Tocado aviso evento diario y prÃ³ximo. ".concat("".concat("".concat("")))))));
					_actualizarregistro();
				}
				else {
		...
		textoexceptodiasdelmes.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (diasseleccionados.size() > 0) {
					transferencias.edit().putString("AcciÃ³n", "Seleccionadas").commit();
					// Created By Prog_mk
					...
					boolean[] checkedItems = new boolean [animals.length];
					 builder.setMultiChoiceItems(animals, checkedItems, new DialogInterface.OnMultiChoiceClickListener() { 
					  ...
					  builder.setPositiveButton("ok", new DialogInterface.OnClickListener() { 
							...
							//evento aceptar
							transferencias.edit().putString("AcciÃ³n lista", "Borrar seleccionadas").commit();
									  //fin 
									  } }); 
					...
					dialog.show();
					if (transferencias.getString("AcciÃ³n lista", "").equals("Borrar seleccionadas")) {
		...
		domingo.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("Criterio dÃ­a de la semana", "domingo").commit();
				}
				else {
				...
				if (transferencias.getString("Buscando", "").equals("No")) {
					if (transferencias.getString("Cargando", "").equals("No")) {
						if (_charSeq.length() > 0) {
							if (_charSeq.equals("(No)")) {
								aviso.setTitle("Aplicaciones");
								aviso.setMessage("El comentario ".concat(_charSeq.concat(" no es vÃ¡lido".concat(""))));
								aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
									@Override
									public void onClick(DialogInterface _dialog, int _which) {
							...
							//estÃ¡ instrucciÃ³n es temporal. Hay que hacer un mÃ³dulo que controle los cambios 
							botonguardarcalendario.setVisibility(View.VISIBLE);
							transferencias.edit().putString("MÃ©todo", "1").commit();
							if (transferencias.getString("MÃ©todo", "").equals("2")) {
								_comprobarcambio();
		...
		button43.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				transferencias.edit().putString("TÃ­tulo", "Horario personalizado ").commit();
		...
		aviso1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("OpciÃ³n", "Aviso").commit();
					cuadrocriteriosaviso.setVisibility(View.VISIBLE);
					//registro 
					_fechayhora();
					transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". OpciÃ³n avisos. Actualizar informe de avisos. ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat(""))))))))).commit();
					if (registro.size() > 0) {
						registro.set((int)0, transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". NÃºmero de datos: ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat("")))))))));
					}
					else {
						registro.add(transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". NÃºmero de datos: ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat("")))))))));
					}
					listviewregistro.setAdapter(new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_list_item_1, registro));
					((BaseAdapter)listviewregistro.getAdapter()).notifyDataSetChanged();
					if (transferencias.getString("Actualizando registro", "").equals("No")) {
						...
						transferencias.edit().putString("Registro", "").commit();
						if (transferencias.getString("Guardar registro automÃ¡ticamente", "").equals("Si")) {
							transferencias.edit().putString("Datos", "").commit();
							transferencias.edit().putString("Contador registro", "0").commit();
							if (registro.size() > 0) {
					...
					transferencias.edit().putString("OpciÃ³n", "No").commit();
				...
				//registro 
				_fechayhora();
				transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Fin de opciÃ³n aviso. ".concat(""))))).commit();
				if (registro.size() > 0) {
					registro.set((int)0, transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". NÃºmero de datos: ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat("")))))))));
				}
				else {
		...
		guardaraviso.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Fin de botÃ³n guardar avisos. Avisos: ".concat(String.valueOf((long)(listaavisoseventospersonalizados.size())).concat("")))))).commit();
		...
		spinneravisoeventos.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("Ãšltimo criterio", "Evento").commit();
				aviso.setTitle(transferencias.getString("Ãšltimo criterio", "").concat("".concat("".concat(""))));
		...
		borraravisoevento.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (avisoevento.size() > 0) {
					avisoevento.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n aviso evento", ""))));
				}
				else {
		...
		spinneravisomomento.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("Ãšltimo criterio", "Momento").commit();
				aviso.setTitle("Ãšltimo criterio ".concat(transferencias.getString("Ãšltimo criterio", "").concat("".concat(""))));
				...
				if (transferencias.getString("MÃ©todo", "").equals("2")) {
					...
					if (transferencias.getString("Aviso correcto", "").equals("Si")) {
						transferencias.edit().putString("AÃ±adir momento", transferencias.getString("Aviso momento", "")).commit();
		...
		borraravisomomento.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				avisomomento.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n aviso momento", ""))));
			}
		});
		...
		spinneravisoforma.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("Ãšltimo criterio", "Forma").commit();
				aviso.setTitle("Ãšltimo criterio ".concat(transferencias.getString("Ãšltimo criterio", "").concat("".concat(""))));
				...
				if (transferencias.getString("MÃ©todo", "").equals("2")) {
					...
					if (transferencias.getString("Aviso correcto", "").equals("Si")) {
						transferencias.edit().putString("AÃ±adir forma", listaavisoformas.get((int)(_position))).commit();
						guardaraviso.setVisibility(View.VISIBLE);
		...
		borraravisoforma.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				avisoforma.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n aviso forma", ""))));
			}
		});
		...
		spinnerfiltrosaviso.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("Ãšltimo criterio", "Filtro").commit();
				_filtroaviso1();
				transferencias.edit().putString("MÃ©todo", "1").commit();
				if (transferencias.getString("MÃ©todo", "").equals("2")) {
					...
					if (transferencias.getString("Aviso correcto", "").equals("Si")) {
						transferencias.edit().putString("AÃ±adir filtro", transferencias.getString("Aviso filtro", "")).commit();
						guardaraviso.setVisibility(View.VISIBLE);
						transferencias.edit().putString("Aviso guardado", "No").commit();
		...
		spinneraudioaviso.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				transferencias.edit().putString("Ãšltimo criterio", "Audio").commit();
				_filtroaviso1();
			}
		...
		eventosseleccionados.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (transferencias.getString("Cargando", "").equals("No")) {
					if (_isChecked) {
						transferencias.edit().putString("Fecha", "Seleccionar dÃ­as del mes").commit();
						transferencias.edit().putString("Seleccionar", "Solo dÃ­as para aviso").commit();
		...
		mostraraplicacion.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (transferencias.getString("Cargando", "").equals("No")) {
					if (_isChecked) {
						transferencias.edit().putString("Cuando mostrar aviso", "Mostrar aplicaciÃ³n").commit();
						if (avisomomento.size() > 0) {
							contador1 = 0;
							for(int _repeat20 = 0; _repeat20 < (int)(avisomomento.size()); _repeat20++) {
								if (contador1 < avisomomento.size()) {
									if (avisomomento.get((int)(contador1)).equals("Mostrar aplicaciÃ³n")) {
										break;
									}
									else {
							...
							if (contador1 == avisomomento.size()) {
								avisomomento.add("Mostrar aplicaciÃ³n");
							...
							//fin 
							transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". OpciÃ³n mostrar al inicio. Mostrar mÃ³dulo prÃ³ximo aviso. ".concat(""))))).commit();
							_actualizarregistroreciente();
							_proximoaviso();
						}
						else {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (transferencias.getString("Cargando", "").equals("No")) {
					if (_isChecked) {
						...
						transferencias.edit().putString("NotificaciÃ³n", "No").commit();
					}
				}
				else {
		...
		filtro.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					diasdelmesexcepcionales.setVisibility(View.GONE);
					solodatos.setVisibility(View.GONE);
					transferencias.edit().putString("Criterios de bÃºsqueda", "No").commit();
		...
		diasdelmesexcepcionales.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("SelecciÃ³n", "Iniciar").commit();
					textoexceptodiasdelmes.setVisibility(View.VISIBLE);
					transferencias.edit().putString("Criterio dÃ­a de la semana", "No").commit();
					transferencias.edit().putString("Seleccionar", "Excepto dÃ­as").commit();
				}
				else {
					transferencias.edit().putString("Fecha", "Comprobar").commit();
					button44.setVisibility(View.GONE);
					transferencias.edit().putString("SelecciÃ³n", "Cancelar").commit();
					textoexceptodiasdelmes.setVisibility(View.GONE);
					textoexceptodiasdelmes.setText("");
					transferencias.edit().putString("Criterio dÃ­a de la semana", "Si").commit();
		...
		button44.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				transferencias.edit().putString("Fecha", "Comprobar").commit();
				button44.setVisibility(View.GONE);
				transferencias.edit().putString("AcciÃ³n", "Fin").commit();
			}
		});
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					if (eventosseleccionados.isChecked()) {
						...
						transferencias.edit().putString("Seleccionar", "Solo dÃ­as").commit();
					}
				}
				else {
					transferencias.edit().putString("Fecha", "Comprobar").commit();
				...
				registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Seleccionar solo dÃ­as. Fecha: ".concat(transferencias.getString("Fecha", "").concat(". Seleccionar: ".concat(transferencias.getString("Seleccionar", "").concat("".concat("")))))))));
				_actualizarregistro();
			}
		});
		...
		filtrotipocita.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					if (transferencias.getString("OpciÃ³n", "").equals("Aviso")) {
						listatemporal.clear();
						// Created By Prog_mk
						AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);
						builder.setTitle("title");
						...
						boolean[] checkedItems = new boolean [animals.length];
						 builder.setMultiChoiceItems(animals, checkedItems, new DialogInterface.OnMultiChoiceClickListener() { 
						  ...
						  builder.setPositiveButton("ok", new DialogInterface.OnClickListener() { 
								...
								//evento aceptar
								transferencias.edit().putString("AcciÃ³n lista", "Seleccionar").commit();
										  //fin 
										  } }); 
						builder.setNegativeButton("ignore", null); 
						// Create and show the alert dialog 
						...
						if (listatemporal.size() > 0) {
							if (avisotipo.size() > 0) {
								//Comprobar si el elemento estÃ¡ repetido 
								contador1 = 0;
							...
							for(int _repeat170 = 0; _repeat170 < (int)(listatemporal.size()); _repeat170++) {
								if (contador1 < listatemporal.size()) {
									//poner en filtro cuando estÃ© terminado 
					...
					transferencias.edit().putString("Criterios de bÃºsqueda", "Si").commit();
				}
				else {
					if (transferencias.getString("OpciÃ³n", "").equals("Aviso")) {
							...
							aviso.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface _dialog, int _which) {
									...
									if (contador1 == descripcionavisofiltrocomentario.size()) {
										...
										aviso.setMessage("El comentario (".concat(dato.concat(") estÃ¡ repetido ".concat(""))));
										aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
											@Override
											public void onClick(DialogInterface _dialog, int _which) {
		...
		criterios.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				final int _position = _param3;
				transferencias.edit().putString("PosiciÃ³n criterio", String.valueOf((long)(_position))).commit();
			}
		...
		button46.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				if (transferencias.getString("Criterio encontrado", "").equals("No")) {
					...
					aviso.setTitle("AÃ±adir criterio".concat(""));
		...
		button47.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				listanombrecriterios.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n criterio", ""))));
				listacriterios.remove((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n criterio", ""))));
				((ArrayAdapter)criterios.getAdapter()).notifyDataSetChanged();
				criterios.setAdapter(new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_spinner_dropdown_item, listanombrecriterios));
		...
		lunes.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("Criterio dÃ­a de la semana", "lunes").commit();
				}
				else {
		...
		martes.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("Criterio dÃ­a de la semana", "martes").commit();
				}
				else {
		...
		miercoles.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Criterio miÃ©rcoles", "miÃ©rcoles").commit();
					transferencias.edit().putString("Criterio dÃ­a de la semana", "miÃ©rcoles").commit();
				}
				else {
					transferencias.edit().putString("Criterio miÃ©rcoles", "No").commit();
					transferencias.edit().putString("Criterio dÃ­a de la semana", "No").commit();
				}
			}
		});
		...
		jueves.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("Criterio dÃ­a de la semana", "jueves").commit();
				}
				else {
		...
		viernes.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					...
					transferencias.edit().putString("Criterio dÃ­a de la semana", "viernes").commit();
				}
				else {
		...
		sabado.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Criterio sÃ¡bado", "sÃ¡bado").commit();
					transferencias.edit().putString("Criterio dÃ­a de la semana", "sÃ¡bado").commit();
				}
				else {
					transferencias.edit().putString("Criterio sÃ¡bado", "No").commit();
					transferencias.edit().putString("Criterio dÃ­a de la semana", "No").commit();
				}
			}
		});
		...
		cargaautomatica.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Cargar configuraciÃ³n automÃ¡ticamente", "Si").commit();
				}
				else {
					transferencias.edit().putString("Cargar configuraciÃ³n automÃ¡ticamente", "No").commit();
				}
		...
		checkbox18.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Guardar configuraciÃ³n automÃ¡ticamente", "Si").commit();
				}
				else {
					transferencias.edit().putString("Guardar configuraciÃ³n automÃ¡ticamente", "No").commit();
				}
				_guardarconfiguracion();
			}
		});
		...
		checkbox19.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Guardar registro automÃ¡ticamente", "Si").commit();
				}
				else {
					transferencias.edit().putString("Guardar registro automÃ¡ticamente", "No").commit();
				}
				if (transferencias.getString("Guardar configuraciÃ³n automÃ¡ticamente", "").equals("Si")) {
					_guardarconfiguracion();
				}
				else {
		...
		checkbox20.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Cargar registro automÃ¡ticamente", "Si").commit();
				}
				else {
					transferencias.edit().putString("Cargar registro automÃ¡ticamente", "No").commit();
				}
				if (transferencias.getString("Guardar configuraciÃ³n automÃ¡ticamente", "").equals("Si")) {
					_guardarconfiguracion();
				}
		...
		checkbox1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				final boolean _isChecked = _param2;
				if (_isChecked) {
					transferencias.edit().putString("Primera lÃ­nea como nombre", "Si").commit();
					primeralineacomonombre = "Si";
				}
				else {
					transferencias.edit().putString("Primera lÃ­nea como nombre", "No").commit();
		...
		limitelineas.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				final int _position = _param3;
				_fechayhora();
				transferencias.edit().putString("Carga rÃ¡pida a partir de datos", listalimitelineas.get((int)(_position))).commit();
				transferencias.edit().putString("PosiciÃ³n carga rÃ¡pida a partir de datos", String.valueOf((long)(_position))).commit();
		...
		textview5.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				textview5.setText("Carga rÃ¡pida a partir de nÃºmero de datos.\nEsta opciÃ³n se activarÃ¡ a partir de 100 datos y provocarÃ¡ que la aplicaciÃ³n se bloquee hasta que termine la carga.\nDatos actuales: ".concat(transferencias.getString("LÃ­neas", "").concat(". Toque aquÃ­ para volver a comprobar. ")));
			}
		});
		...
		button8.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (transferencias.getString("Cambiado", "").equals("Si")) {
					...
					aviso.setMessage("Si continÃºa se perderÃ¡n los Ãºltimos cambios. Â¿Desea continuar?".concat(""));
					aviso.setPositiveButton("Si", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
		...
		button15.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				//enviar texto. a tÃ­tulo. B texto
									...
									if (caracteres < Double.parseDouble(transferencias.getString("Caracteres", ""))) {
										if (slist.size() > 0) {
											if (caracteres == 0) {
												...
												if (transferencias.getString("MÃ©todo", "").equals("2")) {
													...
													for(int _repeat177 = 0; _repeat177 < (int)(maximo); _repeat177++) {
														if (contador < maximo) {
															...
															//dato = c;
															//StringÂ s="hello";Â Â Â Â 
															//for(int i=0; i<str.length();i++){
															//char c = str.charAt(i);
															//dato=dato+c;
															//Â System.out.println("charÂ atÂ "+i+"Â indexÂ is:Â "+c);Â Â 
															//}
															contador++;
														}
														else {
												...
												str = slist.get((int)(caracteres));
												textoprogreso.setText("Convirtiendo carÃ¡cter ".concat(slist.get((int)(caracteres)).concat("/".concat(String.valueOf((long)(slist.size())).concat(" Dato ".concat(str.concat("")))))));
											}
										}
										else {
											FileUtil.writeFile(nombrenota, texto.getText().toString());
											caracteres = Double.parseDouble(transferencias.getString("LÃ­neas", ""));
										}
				...
				if (transferencias.getString("Cargando", "").equals("No")) {
					if (transferencias.getString("Buscando", "").equals("No")) {
						if (_charSeq.equals("")) {
							...
							if (texto.getText().toString().equals(transferencias.getString("Ãšltimo texto", ""))) {
								botonguardar.setVisibility(View.GONE);
							}
							else {
								transferencias.edit().putString("Ãšltimo carÃ¡cter", _charSeq).commit();
								transferencias.edit().putString("Ãšltimo texto", _charSeq).commit();
								...
								if (nombre.getText().toString().equals("")) {
									if (transferencias.getString("Primera lÃ­nea como nombre", "").equals("Si")) {
										str = texto.getText().toString();
										slist.clear();
										//Cargar lista a partir de un texto 
										//arr es matrix tipo array numÃ©rico 
	...
	private void initializeLogic() {
		/* Ãºltima copia 22/11/24
Copia para restaurar la rutina de carga 14/11/24
Todo parece en orden excepto la rutina para cargar archivo que no estÃ¡ bien coordinada. Mejorada coordinaciÃ³n y velocidad de carga de archivos.
Si se abusa del mÃ³dulo registro, bloqueara la aplicaciÃ³n. 
-----------------------
TeÃ³ricamente corregida rutina del mÃ³dulo mostrar registro reciente para que aÃ±ada datos al archivo en vez de recopilar todos y guardarlos. 29/11/24
Comprobado que la rutina de fin de carga de archivo se ejecuta. 29/11/24
Falta la rutina para mostrar la notificaciÃ³n. 
Botones para guardar avisos despuÃ©s de borrar todo aparentemente corregidos. 20/11/24
Rutina de carga aparentemente corregida. 15/11/24
TeÃ³ricamente rutina para guardar avisos completada. 
En proceso de terminar la carga de avisos nuevos 18/10/24
Falta corregir rutina de carga. 
Rutina para guardar avisos teÃ³ricamente corregida. 13/11/24
No guarda bien los datos de los avisos. Probar. 11/11/24
Terminado guardar configuraciÃ³n automÃ¡ticamente, cargar configuraciÃ³n automÃ¡ticamente y guardar registro automÃ¡ticamente 11/11/24
Copia restaurada debido a fallos de carga de datos. 11/11/24
Corregido botÃ³n de guardar oculto sin datos y rutina para leer avisos de audio y aÃ±adida rutina para guardar avisos en matriz permanente que faltaba en el botÃ³n guardar datos temporales. 4/11/24
AÃ±adido botÃ³n para borrar todo y corregidas rutinas relacionadas (no probado) 3/11/24
AÃ±adida rutina para asegurar que el nombre del archivo copiado no sea demasiado largo 19/9/24
18/9/24 aÃ±adido botÃ³n para hacer copias 
2/8/24 aÃ±adidas rutinas para guardar y cargar avisos. Falta corregir el informe de avisos para que muestre los avisos archivados 
28/8/24 aÃ±adida opciÃ³n para mostrar la fecha actual 
26/8/24 configuraciÃ³n supuestamente corregida 
25/8/24: creando rutina para guardar avisos.
20,16,13,10/9,8,7,6,5,4,3,2/8/24,31/7/24. Informe de avisos corregido, 27,26,25, 22/7/24, 21/7/24, 18/7/24,15/7/24, 11/7/24, 17/1/24. 661. Corrigiendo limite de datos en configuraciÃ³n. 
Fallo a partir de la segunda carga de archivo aparentemente corregido. Velocidad de carga teÃ³ricamente corregida, ConfiguraciÃ³n de pantalla y calendario teÃ³ricamente corregidos. AÃ±adiendo comprobaciÃ³n de aviso repetido.
La rutina para cargar el archivo de configuraciÃ³n general estÃ¡ en el mÃ³dulo cargar variables 
 Filtro aparentemente corregido. RenovaciÃ³n de notificaciones en proceso. Corregidos fallos que se mostraban a partir de Android 12.  Falta perfeccionar filtro de bÃºsqueda. Terminada notificaciÃ³n. Reformando criterios de aviso Corregida variable de establecimientos de ruta. AÃ±adida funciÃ³n para obtener el nÃºmero de dÃ­as del mes en el filtro. AÃ±adido criterio solo dÃ­as. AÃ±adida notificaciÃ³n persistente. La rutina para crear la lista de casillas estÃ¡ en textoexceptodiasdelmes. Corregida rutina para guardar solo dÃ­as. Corregida rutina de copia de fechas. AÃ±adida nueva funciÃ³n para copiar pero no ha de sido probada
...
*/
		...
		transferencias.edit().putString("Ruta configuraciÃ³n", transferencias.getString("Ruta 1", "").concat("/configuraciÃ³n")).commit();
		transferencias.edit().putString("Ruta configuraciÃ³n pantalla", transferencias.getString("Ruta 1", "").concat("/configuraciÃ³n pantalla")).commit();
		...
		if (FileUtil.isExistFile(transferencias.getString("Ruta", "").concat("/activo"))) {
			...
			//Esto incluye la rutina para cargar el archivo de configuraciÃ³n general 
			_cargarvariables();
		}
		...
		switch (_requestCode) {
			...
			if (_resultCode == Activity.RESULT_OK) {
				...
				if (transferencias.getString("MÃ©todo", "").equals("2")) {
					...
					textonotas.setText("Nota ".concat(dato.concat(". LÃ­neas: ".concat(String.valueOf((long)(slist.size())).concat("")))));
					transferencias.edit().putString("SecciÃ³n", "Editando").commit();
				}
				else {
	...
	public void onBackPressed() {
		if (transferencias.getString("SecciÃ³n", "").equals("Ficheros")) {
			aviso.setTitle("Â¿Mantener aplicacion en segundo plano o cerrar?");
			aviso.setMessage("".concat("".concat("".concat(""))));
			aviso.setPositiveButton("Cerrar", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface _dialog, int _which) {
		...
		if (transferencias.getString("SecciÃ³n", "").equals("Calendario")) {
			transferencias.edit().putString("Mostrar secciÃ³n", "Ficheros").commit();
			_mostrarseccion();
			_actualizarlistanotas();
			_ocultaropciones();
			transferencias.edit().putString("Mostrar opciÃ³n", "").commit();
			transferencias.edit().putString("SecciÃ³n", "Ficheros").commit();
		}
		else {
			
		}
		if (transferencias.getString("SecciÃ³n", "").equals("Buscar por lista")) {
			checkbox4.setChecked(false);
			transferencias.edit().putString("Mostrar secciÃ³n", "Editar").commit();
			_mostrarseccion();
			transferencias.edit().putString("Texto seleccionado", "").commit();
		}
		else {
		...
		if (transferencias.getString("SecciÃ³n", "").equals("Principal")) {
		...
		if (transferencias.getString("SecciÃ³n", "").equals("Registro")) {
			if (texto.getText().toString().equals("")) {
				transferencias.edit().putString("Mostrar secciÃ³n", "Lista de notas").commit();
				_mostrarseccion();
				transferencias.edit().putString("Mostrar opciÃ³n", "Lista de notas").commit();
				_mostraropcion();
			}
			else {
				transferencias.edit().putString("Mostrar secciÃ³n", "Editar").commit();
		...
		if (transferencias.getString("SecciÃ³n", "").equals("Editar")) {
			if (transferencias.getString("Editando", "").equals("Si")) {
				if (transferencias.getString("Cambiado", "").equals("Si")) {
					...
					aviso.setTitle("Aviso");
					aviso.setMessage("No se ha guardado el Ãºltimo cambio. Si sale de la aplicaciÃ³n se perderÃ¡n los cambios. Â¿Desea salir? ".concat(""));
					aviso.setPositiveButton("Si", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							transferencias.edit().putString("Cambiado", "No").commit();
							transferencias.edit().putString("Cargado", "Si").commit();
							transferencias.edit().putString("SecciÃ³n", "Notas").commit();
							transferencias.edit().putString("Mostrar secciÃ³n", "Lista de notas").commit();
							_mostrarseccion();
							transferencias.edit().putString("Mostrar opciÃ³n", "Lista de notas").commit();
							_mostraropcion();
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, 2, 0, "Cargar configuraciÃ³n de pantalla");
		...
		menu.add(0, 10, 0, "ConfiguraciÃ³n");
		menu.add(0, 11, 0, "InformaciÃ³n");
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (_title.equals("InformaciÃ³n")) {
			aviso.setTitle("InformaciÃ³n");
		...
		if (_title.equals("Cargar configuraciÃ³n de pantalla")) {
		...
		if (_title.equals("Ficheros")) {
			transferencias.edit().putString("Mostrar secciÃ³n", "Ficheros").commit();
			_mostrarseccion();
			transferencias.edit().putString("Mostrar opciÃ³n", "Ficheros").commit();
			_mostraropcion();
		}
		else {
		...
		if (_title.equals("Registro")) {
			transferencias.edit().putString("Mostrar secciÃ³n", "Registro").commit();
			_mostrarseccion();
			transferencias.edit().putString("SecciÃ³n", "Registro").commit();
		...
		if (_title.equals("Abrir fichero")) {
			...
			if (transferencias.getString("Cambiado", "").equals("Si")) {
				aviso.setTitle("Aviso");
				aviso.setMessage("La nota actual ha sido modificada. Si continÃºa se perderÃ¡n los cambios. Â¿Desea continuar? ".concat("".concat("".concat(""))));
				aviso.setPositiveButton("Si", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface _dialog, int _which) {
		...
		if (_title.equals("Calendario")) {
			transferencias.edit().putString("Mostrar opciÃ³n", "Calendario").commit();
			_mostraropcion();
			transferencias.edit().putString("Mostrar secciÃ³n", "Calendario").commit();
			_mostrarseccion();
		}
		else {
			
		}
		if (_title.equals("ConfiguraciÃ³n")) {
			transferencias.edit().putString("Mostrar opciÃ³n", "ConfiguraciÃ³n").commit();
			_mostraropcion();
			transferencias.edit().putString("Mostrar secciÃ³n", "ConfiguraciÃ³n").commit();
			_mostrarseccion();
		}
		else {
	...
	public void _guardarconfiguracion() {
		if (transferencias.getString("Cargando configuraciÃ³n", "").equals("No")) {
			transferencias.edit().putString("ConfiguraciÃ³n", "Primera lÃ­nea como nombre\n".concat(transferencias.getString("Primera lÃ­nea como nombre", "").concat("\n"))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Cargar configuraciÃ³n automÃ¡ticamente\n".concat(transferencias.getString("Cargar configuraciÃ³n automÃ¡ticamente", "").concat("\n")))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Guardar en segundo plano\n".concat(transferencias.getString("Guardar en segundo plano", "").concat("\n")))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Carga rÃ¡pida a partir de datos\n".concat(transferencias.getString("PosiciÃ³n carga rÃ¡pida a partir de datos", "").concat("\n".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("\n")))))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Guardar configuraciÃ³n automÃ¡ticamente\n".concat(transferencias.getString("Guardar configuraciÃ³n automÃ¡ticamente", "").concat("\n")))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Cargar registro automÃ¡ticamente\n".concat(transferencias.getString("Cargar registro automÃ¡ticamente", "").concat("\n")))).commit();
			transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("Guardar registro automÃ¡ticamente\n".concat(transferencias.getString("Guardar registro automÃ¡ticamente", "").concat("\n")))).commit();
			transferencias.edit().putString("MÃ©todo", "1").commit();
			if (transferencias.getString("MÃ©todo", "").equals("2")) {
				transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("(Ancho editar)\n".concat(String.valueOf((long)(anchoeditar)).concat("\n")))).commit();
				transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("(Alto editar)\n".concat(String.valueOf((long)(altoeditar)).concat("\n")))).commit();
				transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("(Lista de notas)\n".concat(transferencias.getString("PosiciÃ³n vertical lista de notas", "").concat("\n")))).commit();
			}
			else {
				
			}
			FileUtil.writeFile(transferencias.getString("Ruta configuraciÃ³n", ""), transferencias.getString("ConfiguraciÃ³n", ""));
			transferencias.edit().putString("ConfiguraciÃ³n cambiada", "No").commit();
			botonguardarconfiguracion.setVisibility(View.INVISIBLE);
			listview1.setBackgroundColor(Color.TRANSPARENT);
			transferencias.edit().putString("Registro", "Guardada configuraciÃ³n. ".concat("")).commit();
			_actualizarregistroreciente();
			SketchwareUtil.showMessage(getApplicationContext(), "ConfiguraciÃ³n guardada ");
		}
		else {
	...
	public void _configurarnotas() {
		...
		_fechayhora();
		veces = 0;
		registro.add("AcciÃ³n nota: ".concat(transferencias.getString("Nombre nota", "").concat(". NÃºmero: ".concat(String.valueOf((long)(contadorconfigurarnota)).concat(". Cargando: ".concat(transferencias.getString("Cargando", "").concat("")))))));
		transferencias.edit().putString("Cambiado1", "No").commit();
		if (transferencias.getString("Nota", "").equals("Cargar")) {
			...
			transferencias.edit().putString("MÃ©todo1", "1").commit();
			if (transferencias.getString("MÃ©todo1", "").equals("2")) {
				principal1.setVisibility(View.GONE);
				opcionestexto.setVisibility(View.GONE);
			...
			transferencias.edit().putString("MÃ©todo", "1").commit();
			if (transferencias.getString("MÃ©todo", "").equals("2")) {
				//leer archivo con barra de programaciÃ³n. No funciona
				temporizador = new TimerTask() {
					@Override
					public void run() {
						runOnUiThread(new Runnable() {
							...
							public void run() {
								...
								//}
								//i++;
								textoprogreso.setText("Leyendo lÃ­neas: ".concat(String.valueOf((long)(0)).concat("")));
								//Thread.sleep(10);
								//}
								parar = true;
							}
			...
			transferencias.edit().putString("LÃ­neas", String.valueOf((long)(slist.size()))).commit();
			//establece nÃºmero de datos en una opciÃ³n de la configuraciÃ³n 
			textview5.setText("Carga rÃ¡pida a partir de nÃºmero de datos.\nEsta opciÃ³n se activarÃ¡ a partir de 100 datos y provocarÃ¡ que la aplicaciÃ³n se bloquee hasta que termine la carga. Datos actuales: ".concat(transferencias.getString("LÃ­neas", "").concat("")));
			transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Cargando archivo (".concat(transferencias.getString("Nombre nota", "").concat("). LÃ­neas: ".concat(String.valueOf((long)(slist.size())).concat(". Mostrar carga a partir de lÃ­neas: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("")))))))))).commit();
			if (slist.size() > 13) {
				...
				progressbar1.setMax((int)100);
				texto.setText("");
				textoprogreso.setText("Cargando lÃ­neas");
				lineas.clear();
				listabuscar.clear();
				listadia.clear();
				listames.clear();
				...
				avisosfiltrotemporal.add("No");
				contadorcargar = 0;
				//Esto controla que mÃ©todo se va a usar para cargar el archivo dependiendo del lÃ­mite de datos 
				if (Double.parseDouble(transferencias.getString("Carga rÃ¡pida a partir de datos", "")) > 0) {
					if ((Double.parseDouble(transferencias.getString("LÃ­neas", "")) == Double.parseDouble(transferencias.getString("Carga rÃ¡pida a partir de datos", ""))) || (Double.parseDouble(transferencias.getString("LÃ­neas", "")) > Double.parseDouble(transferencias.getString("Carga rÃ¡pida a partir de datos", "")))) {
						transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Carga rÃ¡pida. NÃºmero de datos: ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat(""))))))))).commit();
						//registro 
						if (registro.size() > 0) {
							registro.set((int)0, transferencias.getString("Registro", ""));
						}
						...
						transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Carga con temporizador (lenta). NÃºmero de datos: ".concat(transferencias.getString("LÃ­neas", "").concat(". Limite de datos: ".concat(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat(""))))))))).commit();
						temporizadorcargar = new TimerTask() {
							@Override
							public void run() {
								runOnUiThread(new Runnable() {
				...
				textobusqueda.setText("No hay eventos. Datos mÃ­nimos 14. Datos totales: ".concat(String.valueOf((long)(slist.size())).concat("")));
				texto.setText(str);
				textotemporal = texto.getText().toString();
				linearprogreso.setVisibility(View.GONE);
				opcionestexto.setVisibility(View.GONE);
				...
				if (slist.size() == 1) {
					...
					transferencias.edit().putString("Contador lÃ­neas", "0").commit();
					for(int _repeat3317 = 0; _repeat3317 < (int)(slist.size()); _repeat3317++) {
						if (Double.parseDouble(transferencias.getString("Contador lÃ­neas", "")) < slist.size()) {
							transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("\n".concat(slist.get((int)(Double.parseDouble(transferencias.getString("Contador lÃ­neas", ""))))))).commit();
							transferencias.edit().putString("Contador lÃ­neas", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador lÃ­neas", "")) + 1))).commit();
						}
						else {
		...
		if (transferencias.getString("MÃ©todo", "").equals("2")) {
			...
			if (transferencias.getString("Nota", "").equals("Recargar")) {
				transferencias.edit().putString("Mostrar secciÃ³n", "Cargar").commit();
				_mostrarseccion();
				transferencias.edit().putString("Mostrar opciÃ³n", "Cargar").commit();
				_mostraropcion();
				transferencias.edit().putString("Cargado", "No").commit();
				...
				contador = 0;
				parar = false;
				transferencias.edit().putString("LÃ­neas", String.valueOf((long)(lineas.size()))).commit();
				if (lineas.size() > 0) {
					if (lineas.size() > Double.parseDouble(transferencias.getString("Mostrar progreso a partir de lÃ­neas", ""))) {
						progressbar1.setIndeterminate(false);
						transferencias.edit().putString("Mostrar secciÃ³n", "Cargar").commit();
						_mostrarseccion();
						transferencias.edit().putString("Mostrar opciÃ³n", "Cargar").commit();
								...
								runOnUiThread(new Runnable() {
									...
									public void run() {
										if (contador < lineas.size()) {
											...
											linearprogreso.setVisibility(View.GONE);
											transferencias.edit().putString("Ãšltimo texto", texto.getText().toString()).commit();
											transferencias.edit().putString("Cargado", "Si").commit();
											transferencias.edit().putString("LÃ­neas", String.valueOf((long)(lineas.size()))).commit();
											transferencias.edit().putString("Cargando", "No").commit();
											textotemporal = texto.getText().toString();
	...
	public void _fechayhora() {
		...
		transferencias.edit().putString("DÃ­a", dia).commit();
		transferencias.edit().putString("Mes", mes).commit();
		transferencias.edit().putString("AÃ±o", ano).commit();
		...
		transferencias.edit().putString("DÃ­a de la semana", transferencias.getString("EEEE", "")).commit();
		...
		transferencias.edit().putString("Fecha completa", transferencias.getString("DÃ­a", "").concat("-".concat(transferencias.getString("Mes", "").concat("-".concat(transferencias.getString("AÃ±o", "")))))).commit();
		diasmes = calendario.getActualMaximum(Calendar.DATE);
		transferencias.edit().putString("DÃ­as del mes", String.valueOf((long)(diasmes))).commit();
	}
	...
	public void _guardar() {
		...
		if (transferencias.getString("Guardar", "").equals("Si")) {
			...
			//guardar datos
			if (fecha.size() > 0) {
				transferencias.edit().putString("Datos", fecha.get((int)0).get("Datos dÃ­a").toString()).commit();
				contador = 0;
				transferencias.edit().putString("Dato", "(Calendario)").commit();
				for(int _repeat124 = 0; _repeat124 < (int)((fecha.size() - 1)); _repeat124++) {
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(DÃ­a)".concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(fecha.get((int)contador).get("DÃ­a").toString().concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(Mes)".concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(fecha.get((int)contador).get("Mes").toString().concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(AÃ±o)".concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(fecha.get((int)contador).get("AÃ±o").toString().concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat("(Estado)".concat("\n"))).commit();
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(estado.get((int)contador).get("DÃ­a").toString().concat("\n"))).commit();
					...
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(registrobidimensional.get((int)contador).get("DÃ­a").toString().concat("\n"))).commit();
					...
					transferencias.edit().putString("Dato", transferencias.getString("Dato", "").concat(registrobidimensional.get((int)contador).get("AÃ±o").toString().concat("\n"))).commit();
				...
				if (nombre.getText().toString().equals(transferencias.getString("Ãšltimo nombre", ""))) {
					...
					aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface _dialog, int _which) {
							FileUtil.deleteFile(transferencias.getString("Ruta ficheros", "").concat("/".concat(transferencias.getString("Ãšltimo nombre", "").concat(".txt"))));
							SketchwareUtil.showMessage(getApplicationContext(), "Cambiado nombre de nota".concat(""));
							transferencias.edit().putString("Ãšltimo nombre", transferencias.getString("Nombre nota", "")).commit();
						}
					});
			...
			SketchwareUtil.showMessage(getApplicationContext(), "Establezca nombre de nota vÃ¡lido");
		}
	}
	
	
	public void _calculardetalles() {
		if (transferencias.getString("Ãšltimo carÃ¡cter", "").equals("")) {
			...
			if (slist.size() > 0) {
				transferencias.edit().putString("LÃ­neas", String.valueOf((long)(slist.size()))).commit();
				transferencias.edit().putString("Texto primera lÃ­nea", slist.get((int)(0))).commit();
			}
			else {
				if (transferencias.getString("Ãšltimo carÃ¡cter", "").length() > 0) {
					transferencias.edit().putString("Caracteres", String.valueOf((long)(texto.getText().toString().length()))).commit();
					transferencias.edit().putString("Texto primera lÃ­nea", texto.getText().toString()).commit();
				}
				else {
					transferencias.edit().putString("LÃ­neas", "0").commit();
				}
			}
			//fin
			//caracteres
		...
		transferencias.edit().putString("Detalles", "LÃ­neas: ".concat(transferencias.getString("LÃ­neas", "").concat("\nUltimo cambio ".concat(transferencias.getString("Hora completa", "").concat("\nUltimo carÃ¡cter: ".concat(transferencias.getString("Ãšltimo carÃ¡cter", "").concat("\n"))))))).commit();
	...
	public void _buscarymarcar() {
		//meter valor del texto del cuadro de bÃºsqueda en el evento de cambio de texto
		text = texto.getText().toString();
	...
	public void _buscartexto() {
		if (edittext2.getText().toString().equals("")) {
			...
			if (texto.getText().toString().equals("")) {
				...
				if (transferencias.getString("Ãšltima bÃºsqueda", "").equals(edittext2.getText().toString())) {
					//bÃºsqueda siguiente
					if (x < posicionx.size()) {
	...
	public void _rutinas() {
		...
		for(int _repeat25 = 0; _repeat25 < (int)(slist.size()); _repeat25++) {
			if (caracteres < slist.size()) {
				if (slist.get((int)(caracteres)).equals(".")) {
					position = caracteres;
					//margen de error de 1 al sumar. DeberÃ­a funcionar sumando 3
					if ((caracteres + 4) == slist.size()) {
						if (slist.get((int)(caracteres + 1)).equals("t") && (slist.get((int)(caracteres + 2)).equals("x") && slist.get((int)(caracteres + 3)).equals("t"))) {
							caracteres = slist.size();
						}
	...
	public void _recibir() {
		...
		transferencias.edit().putString("MÃ©todo", "2").commit();
	...
	public void _cargarconfiguracion() {
		if (FileUtil.isExistFile(transferencias.getString("Ruta configuraciÃ³n", ""))) {
			//cargar configuraciÃ³n
			textocargar.setText("Cargando configuraciÃ³n ");
			slist.clear();
			str = FileUtil.readFile(transferencias.getString("Ruta configuraciÃ³n", ""));
			String [] arr = str.split("\n");
			for (int i = 0; i < arr.length; i++) {
				slist.add(arr[i]);
			}
			transferencias.edit().putString("Contador configuraciÃ³n", "0").commit();
			for(int _repeat26 = 0; _repeat26 < (int)(slist.size()); _repeat26++) {
				if (Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) < slist.size()) {
					if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Cargar configuraciÃ³n automÃ¡ticamente")) {
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("Cargar configuraciÃ³n automÃ¡ticamente", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Si")) {
					...
					if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Carga rÃ¡pida a partir de datos")) {
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("PosiciÃ³n carga rÃ¡pida a partir de datos", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						limitelineas.setSelection((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n carga rÃ¡pida a partir de datos", ""))));
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("Carga rÃ¡pida a partir de datos", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						aviso.setTitle("Limite de datos ");
						aviso.setMessage(transferencias.getString("Carga rÃ¡pida a partir de datos", "").concat("".concat("".concat(""))));
						aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface _dialog, int _which) {
					...
					//al parecer, la carga de archivos falla si se cargan mÃ¡s opciones aquÃ­ 
					if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Guardar registro automÃ¡ticamente")) {
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("Guardar registro automÃ¡ticamente", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						if (transferencias.getString("Guardar registro automÃ¡ticamente", "").equals("Si")) {
					...
					if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Cargar registro automÃ¡ticamente")) {
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("Cargar registro automÃ¡ticamente", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						if (transferencias.getString("Cargar registro automÃ¡ticamente", "").equals("Si")) {
					...
					if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("Guardar configuraciÃ³n automÃ¡ticamente")) {
						transferencias.edit().putString("Contador configuraciÃ³n", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")) + 1))).commit();
						transferencias.edit().putString("Guardar configuraciÃ³n automÃ¡ticamente", slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", ""))))).commit();
						if (transferencias.getString("Guardar configuraciÃ³n automÃ¡ticamente", "").equals("Si")) {
					...
					transferencias.edit().putString("MÃ©todo", "1").commit();
					if (transferencias.getString("MÃ©todo", "").equals("2")) {
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("(Alto editar)")) {
							
						}
						else {
							
						}
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("(PosiciÃ³n barra de herramientas vertical)")) {
							
						}
						else {
							
						}
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("(PosiciÃ³n barra de herramientas horizontal)")) {
							
						}
						else {
							
						}
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador configuraciÃ³n", "")))).equals("(Mostrar progreso por debajo de lÃ­neas)")) {
			...
			SketchwareUtil.showMessage(getApplicationContext(), "ConfiguraciÃ³n cargada");
			textview2.setText("ConfiguraciÃ³n cargada. ");
			button59.setVisibility(View.GONE);
		}
		else {
			SketchwareUtil.showMessage(getApplicationContext(), "No existen datos de configuraciÃ³n");
			textview2.setText("ConfiguraciÃ³n. (no existe archivo de configuraciÃ³n)");
		}
		transferencias.edit().putString("Cargando configuraciÃ³n", "No").commit();
		transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". Fin de carga de configuraciÃ³n. ".concat(""))))).commit();
		_actualizarregistroreciente();
	}
	...
	public void _cargarpantalla() {
		...
		//preparar conversiÃ³n
		//char valorcaracter = â€˜Bâ€™;
		//int asci = (int) valorcaracter;
		_ocultaropciones();
	...
	public void _moverabajo() {
		transferencias.edit().putString("DirecciÃ³n", "Abajo").commit();
		if (transferencias.getString("Parte", "").equals("Cuadro de ediciÃ³n")) {
			if (transferencias.getString("Tipo configuraciÃ³n", "").equals("Ancho")) {
		...
		if (transferencias.getString("Parte", "").equals("Lista de notas")) {
			if (transferencias.getString("Dimensiones", "").equals("PosiciÃ³n")) {
				if (transferencias.getString("DirecciÃ³n", "").equals("Abajo")) {
				...
				if (transferencias.getString("DirecciÃ³n", "").equals("Arriba")) {
					posicionvertical = posicionvertical - contadormover;
					listview1.setTranslationY((float)(posicionvertical));
				}
				else {
		...
		contadormover = 0;
		textodimensiones.setText("Parte: ".concat(transferencias.getString("Parte", "").concat(". ".concat(transferencias.getString("Tipo configuraciÃ³n", "").concat(": ".concat(String.valueOf((long)(anchoeditar)).concat("")))))));
		_guardarconfiguracion();
	}
	...
	public void _moverarriba() {
		...
		if (transferencias.getString("Parte", "").equals("Cuadro de ediciÃ³n")) {
			...
			if (transferencias.getString("Tipo configuraciÃ³n", "").equals("Alto")) {
			...
			if (transferencias.getString("Tipo configuraciÃ³n", "").equals("Mover")) {
		...
		textodimensiones.setText("Parte: ".concat(transferencias.getString("Parte", "").concat(". ".concat(transferencias.getString("Tipo configuraciÃ³n", "").concat(": ".concat(String.valueOf((long)(altoeditar)).concat("")))))));
		_guardarconfiguracion();
	}
	...
	public void _controlopciones() {
		_ocultaropciones();
		barraherramientas.setVisibility(View.VISIBLE);
		if (transferencias.getString("SecciÃ³n opciones", "").equals("Buscar calendario")) {
			transferencias.edit().putString("Mostrar opciÃ³n", "Calendario").commit();
			_mostraropcion();
		}
		else {
		...
		if (transferencias.getString("SecciÃ³n opciones", "").equals("OpciÃ³n calendario")) {
			transferencias.edit().putString("Mostrar opciÃ³n", "Organizar").commit();
			_mostraropcion();
		}
		else {
		...
		if (transferencias.getString("SecciÃ³n opciones", "").equals("Editar")) {
			edicion.setVisibility(View.VISIBLE);
			edicion.setTranslationY((float)(-50));
		...
		if (transferencias.getString("SecciÃ³n opciones", "").equals("Notas")) {
			cuadroficheros.setVisibility(View.VISIBLE);
	...
	public void _ventana() {
		//Modulo creado para intentar eliminar el cÃ³digo que falla en el archivo java 
	}
	...
	public void _moverposicion() {
		...
		if (transferencias.getString("Parte", "").equals("Lista de notas")) {
			if (transferencias.getString("Tipo configuraciÃ³n", "").equals("PosiciÃ³n")) {
				if (transferencias.getString("DirecciÃ³n", "").equals("Abajo")) {
					posicionvertical = posicionvertical + contadormover;
				}
				else {
				...
				listview1.setTranslationY((float)(posicionvertical));
				textodimensiones.setText("Parte: ".concat(transferencias.getString("Parte", "").concat(". ".concat(transferencias.getString("Tipo configuraciÃ³n", "").concat(": ".concat(String.valueOf((long)(posicionvertical)).concat("")))))));
				transferencias.edit().putString("PosiciÃ³n vertical lista de notas", String.valueOf((long)(posicionvertical))).commit();
			}
			else {
	...
	public void _mostrarseccion() {
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("Ficheros")) {
			principal1.setVisibility(View.VISIBLE);
			buscarlista.setVisibility(View.GONE);
			seccioncalendario.setVisibility(View.GONE);
			editar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("Editar")) {
			principal1.setVisibility(View.GONE);
			buscarlista.setVisibility(View.GONE);
			seccioncalendario.setVisibility(View.GONE);
			editar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("Registro")) {
			principal1.setVisibility(View.GONE);
			buscarlista.setVisibility(View.GONE);
			seccioncalendario.setVisibility(View.GONE);
			editar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("Buscar por lista")) {
			principal1.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("Calendario")) {
			transferencias.edit().putString("SecciÃ³n", "Calendario").commit();
			principal1.setVisibility(View.GONE);
			buscarlista.setVisibility(View.GONE);
			seccioncalendario.setVisibility(View.GONE);
			editar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar secciÃ³n", "").equals("ConfiguraciÃ³n")) {
			transferencias.edit().putString("SecciÃ³n", "ConfiguraciÃ³n").commit();
			principal1.setVisibility(View.GONE);
			buscarlista.setVisibility(View.GONE);
			seccioncalendario.setVisibility(View.GONE);
			editar.setVisibility(View.GONE);
	...
	public void _mostraropcion() {
		...
		barraherramientas.setVisibility(View.GONE);
		cuadrobuscar.setVisibility(View.GONE);
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Calendario")) {
		...
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Ficheros")) {
			barraherramientas.setVisibility(View.VISIBLE);
			cuadropantalla.setVisibility(View.GONE);
			cuadroficheros.setVisibility(View.VISIBLE);
			cuadrobuscar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Editar")) {
			barraherramientas.setVisibility(View.VISIBLE);
			cuadropantalla.setVisibility(View.GONE);
			cuadroficheros.setVisibility(View.GONE);
			cuadrobuscar.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Buscar")) {
			barraherramientas.setVisibility(View.VISIBLE);
			cuadropantalla.setVisibility(View.GONE);
			cuadroficheros.setVisibility(View.GONE);
			edicion.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Cargar")) {
			barraherramientas.setVisibility(View.VISIBLE);
			cuadrocargar.setVisibility(View.VISIBLE);
			cuadropantalla.setVisibility(View.GONE);
			cuadroficheros.setVisibility(View.GONE);
		...
		if (transferencias.getString("Mostrar opciÃ³n", "").equals("Organizar")) {
			barraherramientas.setVisibility(View.VISIBLE);
		...
		registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(" OpciÃ³n ".concat(transferencias.getString("Mostrar opciÃ³n", "").concat("".concat("")))))));
		_actualizarregistro();
	}
	...
	public void _cargarfichero() {
		if (transferencias.getString("Rutina", "").equals("Fin")) {
			...
			if (transferencias.getString("Tarea", "").equals("(Calendario)")) {
				transferencias.edit().putString("Mostrar secciÃ³n", "Calendario").commit();
				_mostrarseccion();
				if (slist.get((int)(contadorcargar)).equals("(DÃ­a)")) {
					contadorcargar++;
					listadia.add(Double.valueOf(Double.parseDouble(slist.get((int)(contadorcargar)))));
					if (slist.get((int)(contadorcargar)).equals(transferencias.getString("DÃ­a", ""))) {
						transferencias.edit().putString("DÃ­a seleccionado", transferencias.getString("DÃ­a", "")).commit();
					}
					else {
				...
				if (slist.get((int)(contadorcargar)).equals("(AÃ±o)")) {
					contadorcargar++;
					listaano.add(Double.valueOf(Double.parseDouble(slist.get((int)(contadorcargar)))));
					if (slist.get((int)(contadorcargar)).equals(transferencias.getString("AÃ±o", ""))) {
						transferencias.edit().putString("AÃ±o seleccionado", transferencias.getString("AÃ±o", "")).commit();
					}
					else {
				...
				if (slist.get((int)(contadorcargar)).equals("(Estado)")) {
					...
					if (transferencias.getString("DÃ­a seleccionado", "").equals(transferencias.getString("DÃ­a", ""))) {
						if (transferencias.getString("Mes seleccionado", "").equals(transferencias.getString("Mes", ""))) {
							if (transferencias.getString("AÃ±o seleccionado", "").equals(transferencias.getString("AÃ±o", ""))) {
		...
		if (transferencias.getString("Rutina", "").equals("Fin")) {
			//. Fin de carga de archivo. Comprueba si se ha leÃ­do toda la rutina de carga en esta rutina y la rutina cargar fichero 1 en cada bucle y luego si el bucle ha finalizado 
			contadorcargar++;
			if ((contadorcargar == slist.size()) || (contadorcargar > slist.size())) {
				//registro
				transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat(". MÃ³dulo cargar fichero. Fin de carga de archivo (".concat(transferencias.getString("Nombre nota", "").concat("). Datos (".concat(String.valueOf((long)(contadorcargar + 1)).concat("/".concat(String.valueOf((long)(slist.size())).concat(") ")))))))))).commit();
				_actualizarregistroreciente();
				//fin de registro 
				if (veces < 1) {
					...
					_proximoaviso();
					transferencias.edit().putString("Cargando", "No").commit();
					if (Double.parseDouble(transferencias.getString("LÃ­neas", "")) < Double.parseDouble(transferencias.getString("Carga rÃ¡pida a partir de datos", ""))) {
						temporizadorcargar.cancel();
					}
					else {
		...
		if (transferencias.getString("MÃ©todo", "").equals("2")) {
			...
			for(int _repeat24 = 0; _repeat24 < (int)(slist.size()); _repeat24++) {
				if (contador < slist.size()) {
					if (slist.get((int)(contador)).equals("DÃ­as de la semana")) {
						contador++;
					}
					else {
	...
	public void _comprobarfecha() {
		transferencias.edit().putString("PosiciÃ³n", "0").commit();
		transferencias.edit().putString("Fecha", "Comprobar").commit();
		transferencias.edit().putString("Cambiado", "No").commit();
		transferencias.edit().putString("Encontrado", "No").commit();
		transferencias.edit().putString("Buscando", "Si").commit();
		if (listadia.size() > 0) {
			...
			for(int _repeat22 = 0; _repeat22 < (int)(listadia.size()); _repeat22++) {
				if (contador < listadia.size()) {
					...
					if ((listadia.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a seleccionado", ""))) && ((listames.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes seleccionado", ""))) && (listaano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o seleccionado", ""))))) {
						...
						transferencias.edit().putString("Hora desde", String.valueOf((long)(horadesde.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue()))).commit();
						transferencias.edit().putString("Hora hasta", String.valueOf((long)(horahasta.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue()))).commit();
						transferencias.edit().putString("Ãšltimo suceso", listaestado.get((int)(contador))).commit();
						transferencias.edit().putString("Ãšltima hora desde", String.valueOf((long)(horadesde.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue()))).commit();
						transferencias.edit().putString("Ãšltima hora hasta", String.valueOf((long)(horahasta.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue()))).commit();
						break;
					}
					else {
			...
			aviso.setTitle("PosiciÃ³n");
			aviso.setMessage(transferencias.getString("PosiciÃ³n", "").concat("".concat("".concat(""))));
			aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface _dialog, int _which) {
			...
			if (transferencias.getString("Encontrado", "").equals("No")) {
				//textview junto al textview bÃºsqueda que muestra si existe registro del evento y cuantos hay 
				...
				personalizado1.setVisibility(View.GONE);
				borrarevento.setVisibility(View.GONE);
				if (transferencias.getString("BÃºsqueda", "").equals("No")) {
					
				}
				else {
					
				}
				registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". No existe fecha seleccionada: ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat(""))))))))));
				aviso.setTitle("Aplicaciones");
				...
				borrarevento.setVisibility(View.VISIBLE);
				textobusqueda.setText("Registro ".concat(String.valueOf((long)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).concat("/".concat(String.valueOf((long)(listadia.size())).concat("")))));
				if (transferencias.getString("BÃºsqueda", "").equals("No")) {
				...
				opciones.setVisibility(View.VISIBLE);
				if (listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).equals("Ocupado parcial")) {
					cuadrohorario.setVisibility(View.VISIBLE);
					if (listanombrehorario.size() > 0) {
						horariopersonalizado.setVisibility(View.VISIBLE);
				...
				for(int _repeat113 = 0; _repeat113 < (int)(listatiposuceso.size()); _repeat113++) {
					if (contador < listatiposuceso.size()) {
						if (listatiposuceso.get((int)(contador)).equals(listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))))) {
				...
				if (comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).equals("(No)")) {
					comentario.setText("");
					comentario.setHint("Sin comentario");
				}
				else {
					transferencias.edit().putString("Comentario", comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))))).commit();
					transferencias.edit().putString("Ãšltimo comentario", comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", ""))))).commit();
					comentario.setText(comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))));
					aviso.setTitle("Aplicaciones");
					aviso.setMessage("".concat("".concat("".concat(""))));
					aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
						@Override
				...
				if (comentarios.size() == listadia.size()) {
					if (Double.parseDouble(transferencias.getString("PosiciÃ³n", "")) < comentarios.size()) {
						transferencias.edit().putString("MÃ©todo", "1").commit();
						if (transferencias.getString("MÃ©todo", "").equals("1")) {
				...
				for(int _repeat168 = 0; _repeat168 < (int)(listahorariodesde.size()); _repeat168++) {
					if (contador < listahorariodesde.size()) {
						if (listahorariodesde.get((int)(contador)).equals(String.valueOf((long)(horadesde.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())))) {
				...
				for(int _repeat211 = 0; _repeat211 < (int)(listahorariohasta.size()); _repeat211++) {
					if (contador < listahorariohasta.size()) {
						if (listahorariohasta.get((int)(contador)).equals(String.valueOf((long)(horahasta.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())))) {
				...
				dato = transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat("Fecha comprobada. DÃ­a de la semana: ".concat(transferencias.getString("DÃ­a de la semana seleccionado", "").concat(". Fecha seleccionada: ".concat(transferencias.getString("DÃ­a seleccionado", "").concat("/".concat(transferencias.getString("Mes seleccionado", "").concat("/".concat(transferencias.getString("AÃ±o seleccionado", "").concat(". ")))))))))));
				dato = dato.concat(". Fecha encontrada: ".concat(transferencias.getString("Encontrado", "").concat(" ")));
				if (transferencias.getString("Encontrado", "").equals("Si")) {
					dato = dato.concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n", "")))).doubleValue())).concat(". PosiciÃ³n: ".concat(transferencias.getString("PosiciÃ³n", "").concat("/".concat(String.valueOf((long)(listadia.size())).concat(""))))))))));
				}
				else {
			...
			SketchwareUtil.showMessage(getApplicationContext(), "Base de datos vacÃ­a".concat("".concat("".concat("".concat("".concat("".concat("".concat(""))))))));
		}
		transferencias.edit().putString("Buscando", "No").commit();
		if (transferencias.getString("Encontrado", "").equals("Si")) {
		...
		if (transferencias.getString("MÃ©todo", "").equals("2")) {
			if ((0 < fecha.size()) && ((0 < fecha.size()) && (0 < fecha.size()))) {
				if (fecha.get((int)0).get("DÃ­a").toString().equals("No")) {
					...
					if (estado.get((int)0).get("Mes").toString().equals("No")) {
						if (estado.get((int)0).get("AÃ±o").toString().equals("No")) {
					...
					transferencias.edit().putString("DÃ­a deducido", String.valueOf((long)(0))).commit();
					transferencias.edit().putString("Mes deducido", fecha.get((int)0).get("DÃ­a").toString()).commit();
					transferencias.edit().putString("AÃ±o deducido", fecha.get((int)0).get("Mes").toString()).commit();
					registro.add("Fecha deducida ".concat(String.valueOf((long)(0)).concat("/".concat(registrobidimensional.get((int)0).get("DÃ­a").toString().concat("/".concat(registrobidimensional.get((int)0).get("Mes").toString().concat("")))))));
					contador = 0;
					for(int _repeat322 = 0; _repeat322 < (int)((listatiposuceso.size() - 1)); _repeat322++) {
						if (estado.get((int)0).get("DÃ­a").toString().equals(String.valueOf((long)(0)))) {
							if (estado.get((int)0).get("Mes").toString().equals(String.valueOf((long)(0)))) {
								if (estado.get((int)0).get("AÃ±o").toString().equals("Si")) {
					...
					for(int _repeat350 = 0; _repeat350 < (int)((ficheros.size() - 1)); _repeat350++) {
						if (registrobidimensional.get((int)0).get("DÃ­a").toString().equals(String.valueOf((long)(0)))) {
							if (registrobidimensional.get((int)0).get("Mes").toString().equals(String.valueOf((long)(0)))) {
								if (registrobidimensional.get((int)0).get("AÃ±o").toString().equals(ficheros.get((int)(contador)))) {
									spunnerdesdehora.setSelection((int)(Double.parseDouble(registrobidimensional.get((int)0).get("AÃ±o").toString())));
		...
		if (transferencias.getString("Ãšltimo dÃ­a", "").equals(String.valueOf((long)(0)))) {
	...
	public void _ventanaintroducirtexto() {
		aviso.setTitle(transferencias.getString("TÃ­tulo", ""));
		...
		aviso.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface _dialog, int _which) {
				...
				if (edittext.getText().toString().equals("")) {
					...
					editartexto.setMessage("El nombre o dato no es vÃ¡lido ".concat("".concat("".concat(""))));
	...
	public void _ocultar() {
		...
		// AsegÃºrate de que no haya mÃ¡rgenes o padding adicionales
		...
		//este bloque debe ser el Ãºltimo 
		opciones.setVisibility(View.VISIBLE);
	}
	...
	public void _manejarfecha() {
		...
		if (transferencias.getString("Sustituir fecha", "").equals("Si")) {
			...
			horahasta.set((int)contador, Double.parseDouble(transferencias.getString("Hora hasta", "")));
			horarioseleccionado.set((int)contador, transferencias.getString("Horario seleccionado", ""));
			registro.add(transferencias.getString("Fecha completa", "").concat(" - ".concat(transferencias.getString("Hora completa", "").concat(". Sustituido ".concat(String.valueOf((long)(listadia.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(" por ".concat(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat("\n\n".concat("".concat("".concat("")))))))))))))))))));
		}
		else {
	...
	public void _buscar() {
		transferencias.edit().putString("DÃ­a seleccionado", String.valueOf((long)(diaencontrado.get((int)(contador)).doubleValue()))).commit();
		transferencias.edit().putString("Mes seleccionado", String.valueOf((long)(mesencontrado.get((int)(contador)).doubleValue()))).commit();
		transferencias.edit().putString("AÃ±o seleccionado", String.valueOf((long)(anoencontrado.get((int)(contador)).doubleValue()))).commit();
		_comprobarfecha();
	...
	public void _proximoaviso() {
		...
		if (transferencias.getString("Aviso", "").equals("Si")) {
			if (listadia.size() > 0) {
				...
				transferencias.edit().putString("PosiciÃ³n de aviso", "-1").commit();
				dato = "";
				contador = 0;
				for(int _repeat32 = 0; _repeat32 < (int)(listadia.size()); _repeat32++) {
					if (contador < listadia.size()) {
						if (listadia.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("DÃ­a", ""))) {
							if (listames.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes", ""))) {
								if (listaano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o", ""))) {
									transferencias.edit().putString("Aviso encontrado", "Si").commit();
									transferencias.edit().putString("PosiciÃ³n de aviso", String.valueOf((long)(contador))).commit();
									break;
								}
								else {
				...
				if (transferencias.getString("Aviso encontrado", "").equals("No")) {
					dato = transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" Hoy no hay eventos. ".concat(""))))));
					mensaje.add(transferencias.getString("DÃ­a", "").concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" Hoy no hay eventos. ".concat("")))))));
				}
				else {
					if (transferencias.getString("Evento prÃ³ximo", "").equals("Si")) {
						dato = "Hoy. ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat(". "))))));
						mensaje.add("Hoy. ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat(". ")))))));
						if (Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")) < listaestado.size()) {
							dato = dato.concat(listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("\n")));
							mensaje.add("Estado: ".concat(listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("\n"))));
						}
						else {
							dato = dato.concat("Estado no encontrado".concat(". ".concat("")));
							mensaje.add("Estado no encontrado".concat(". ".concat("")));
						}
						if (Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")) < comentarios.size()) {
							if (comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).equals("(No)")) {
								dato = dato.concat("Sin comentario. ".concat(""));
								mensaje.add("Sin comentario. ".concat(""));
							}
							else {
								dato = dato.concat(comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("\n")));
								mensaje.add("Comentario: ".concat(comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("\n"))));
							}
						}
						else {
						...
						transferencias.edit().putString("MÃ©todo", "1").commit();
						if (transferencias.getString("MÃ©todo", "").equals("2")) {
							dato = dato.concat("Horario: ".concat(String.valueOf((long)(horadesde.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat(":".concat(String.valueOf((long)(horahasta.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("\n")))));
							dato = dato.concat(comentarios.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("\n")));
						}
						else {
				...
				for(int _repeat165 = 0; _repeat165 < (int)(listadia.size()); _repeat165++) {
					if (listadia.get((int)(contador)).doubleValue() == (Double.parseDouble(transferencias.getString("DÃ­a", "")) + 1)) {
						if (listames.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("Mes", ""))) {
							if (listaano.get((int)(contador)).doubleValue() == Double.parseDouble(transferencias.getString("AÃ±o", ""))) {
								transferencias.edit().putString("Aviso encontrado", "Si").commit();
								transferencias.edit().putString("PosiciÃ³n de aviso", String.valueOf((long)(contador))).commit();
				...
				//Comprobar si el dÃ­a es mayor que dÃ­as del mes 
				if (transferencias.getString("Aviso encontrado", "").equals("No")) {
					dato = dato.concat(String.valueOf((long)(Double.parseDouble(transferencias.getString("DÃ­a", "")) + 1)).concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" MaÃ±ana no hay eventos. ".concat("")))))));
					mensaje.add(String.valueOf((long)(Double.parseDouble(transferencias.getString("DÃ­a", "")) + 1)).concat("/".concat(transferencias.getString("Mes", "").concat("/".concat(transferencias.getString("AÃ±o", "").concat(" MaÃ±ana no hay eventos. ".concat("")))))));
				}
				else {
					if (listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue() > 31) {
						contador--;
					}
					else {
					...
					if (transferencias.getString("Evento prÃ³ximo", "").equals("Si")) {
						dato = dato.concat(" MaÃ±ana. ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(". ")))))));
						mensaje.add(" MaÃ±ana. ".concat(String.valueOf((long)(listadia.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).doubleValue())).concat("/".concat(String.valueOf((long)(listames.get((int)(contador)).doubleValue())).concat("/".concat(String.valueOf((long)(listaano.get((int)(contador)).doubleValue())).concat(". ")))))));
						if (Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")) < listaestado.size()) {
							dato = dato.concat(listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat("")));
							mensaje.add("Estado: ".concat(listaestado.get((int)(Double.parseDouble(transferencias.getString("PosiciÃ³n de aviso", "")))).concat(". ".concat(""))));
						}
			...
			if (eventoproximo.isChecked()) {
				if (mostraraplicacion.isChecked()) {
					if (transferencias.getString("NotificaciÃ³n", "").equals("Si")) {
						_notificaciones();
		...
		registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Aviso fecha: ".concat(transferencias.getString("Aviso", "").concat(". Evento prÃ³ximo: ".concat(transferencias.getString("Evento prÃ³ximo", "").concat(". NotificaciÃ³n: ".concat(transferencias.getString("NotificaciÃ³n", "").concat(". Aviso encontrado: ".concat(transferencias.getString("Aviso encontrado", "").concat("".concat("")))))))))))));
		_actualizarregistro();
		contador = 0;
		for(int _repeat675 = 0; _repeat675 < (int)(mensaje.size()); _repeat675++) {
			if (contador < mensaje.size()) {
	...
	public void _copias() {
		...
		if (listadia.size() > 0) {
			...
			registro.add(transferencias.getString("Fecha completa", "").concat(". ".concat(transferencias.getString("Hora completa", "").concat(". Buscando. Criterios: ".concat(transferencias.getString("Criterios de bÃºsqueda", "").concat(". Criterio comentario: ".concat(transferencias.getString("Filtro comentario", "").concat(". Fechas encontradas: ".concat(String.valueOf((long)(fechasencontradas.size())).concat("".concat("".concat(""))))))))))));
			if (diaencontrado.size() > 0) {
				contadorbuscar = 0;
				spinnerbusqueda.setAdapter(new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_spinner_dropdown_item, fechasencontradas));
				((ArrayAdapter)spinnerbusqueda.getAdapter()).notifyDataSetChanged();
		...
		if (slist.get((int)(contadorcargar)).equals("(Calendario)")) {
			...
			_mostrarseccion();
			aviso.setTitle("Aplicaciones");
			aviso.setMessage("Se muestra secciÃ³n calendario ".concat("".concat("".concat(""))));
			aviso.setPositiveButton("Vale ", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface _dialog, int _which) {
				...
				if (slist.get((int)(contadorcargar)).equals("(Aviso)")) {
					if (transferencias.getString("Aviso cargado", "").equals("No")) {
						...
						if (slist.get((int)(contadorcargar)).equals("(Criterio aviso)")) {
							...
							if (transferencias.getString("Criterio aviso", "").equals("PrÃ³ximo")) {
						...
						if (slist.get((int)(contadorcargar)).equals("(NotificaciÃ³n)")) {
							contadorcargar++;
							transferencias.edit().putString("NotificaciÃ³n", slist.get((int)(contadorcargar))).commit();
							if (transferencias.getString("NotificaciÃ³n", "").equals("Si")) {
						...
						registro.add("(".concat(transferencias.getString("Fecha completa", "").concat(" - ".concat(transferencias.getString("Hora completa", "").concat(") Aviso: ".concat(transferencias.getString("Aviso", "").concat(". Criterio de aviso: ".concat(transferencias.getString("Criterio aviso", "").concat(". NotificaciÃ³n: ".concat(transferencias.getString("NotificaciÃ³n", "").concat(". Ventana: ".concat(transferencias.getString("Ventana", "").concat(". NÃºmero de criterios: ".concat(transferencias.getString("Criterios", "").concat("".concat(""))))))))))))))));
					}
					else {
	...
	public void _cargarfichero1() {
		...
		if (slist.get((int)(contadorcargar)).equals("(Cuando mostrar aviso)")) {
			...
			if (transferencias.getString("Cuando mostrar aviso", "").equals("Mostrar aplicaciÃ³n")) {
		...
		if (slist.get((int)(contadorcargar)).equals("(Evento prÃ³ximo)")) {
			contadorcargar++;
			transferencias.edit().putString("Evento prÃ³ximo", slist.get((int)(contadorcargar))).commit();
			if (transferencias.getString("Evento prÃ³ximo", "").equals("Si")) {
		...
		if (transferencias.getString("MÃ©todo", "").equals("2")) {
			if (Double.parseDouble(transferencias.getString("Eventos aviso", "")) > 0) {
				...
				transferencias.edit().putString("Fase", "1").commit();
				contadorcargar++;
				//Cargar en cada matriz de avisos el mismo nÃºmero de elementos que se eventos 
	...
	public void _cargarpantalla1() {
		...
		transferencias.edit().putString("Ãšltimo texto", texto.getText().toString()).commit();
		transferencias.edit().putString("Buscando", "No").commit();
		transferencias.edit().putString("Ãšltima bÃºsqueda", "").commit();
		transferencias.edit().putString("Primera bÃºsqueda", "Si").commit();
		transferencias.edit().putString("Ruta convertir fichero", notas).commit();
		transferencias.edit().putString("Cargando", "No").commit();
		transferencias.edit().putString("Reiniciado", "No").commit();
		transferencias.edit().putString("Mostrar progreso a partir de lÃ­neas", "5000").commit();
		transferencias.edit().putString("SecciÃ³n", "Principal").commit();
		transferencias.edit().putString("Importar", "No").commit();
		...
		transferencias.edit().putString("Encriptar", "No").commit();
		transferencias.edit().putString("Guardar configuraciÃ³n automÃ¡ticamente", "No").commit();
		transferencias.edit().putString("ConfiguraciÃ³n", "").commit();
		transferencias.edit().putString("Cargar automÃ¡ticamente", "No").commit();
		transferencias.edit().putString("ConfiguraciÃ³n cambiada", "No").commit();
		...
		transferencias.edit().putString("Ãšltimo dÃ­a", "0").commit();
		transferencias.edit().putString("Ãšltimo mes", "0").commit();
		transferencias.edit().putString("Ãšltimo aÃ±o", "0").commit();
		transferencias.edit().putString("Encontrado", "No").commit();
		...
		transferencias.edit().putString("Filtro", "No").commit();
		transferencias.edit().putString("Solo dÃ­as de la semana", "No").commit();
		transferencias.edit().putString("Fecha", "Comprobar").commit();
		transferencias.edit().putString("Criterio dÃ­a de la semana", "No").commit();
		transferencias.edit().putString("DÃ­a de la semana encontrado", "No").commit();
		transferencias.edit().putString("Criterio lunes", "No").commit();
		transferencias.edit().putString("Criterio martes", "No").commit();
		transferencias.edit().putString("Criterio miÃ©rcoles", "No").commit();
		transferencias.edit().putString("Criterio jueves", "No").commit();
		...
		if (transferencias.getString("MÃ©todo", "").equals("2")) {
			...
			transferencias.edit().putString("MÃ©todo", "1").commit();
			if (transferencias.getString("MÃ©todo", "").equals("2")) {
				//mÃ©todo 3. //Cambiar tamaÃ±o de linear y teÃ³ricamente cualquier herramienta. Crear en un MÃ³dulo. Nombre. Variable numÃ©rica etiqueta : ancho. AÃ±adir. Ancho. AÃ±adir. Etiqueta alto. AÃ±adir. Nombre variable alto. AÃ±adir. Variable vista. Nombre variable vista. AÃ±adir.
		...
		if (FileUtil.isExistFile(transferencias.getString("Ruta", "").concat("/datos.txt"))) {
			...
			for(int _repeat202 = 0; _repeat202 < (int)(slist.size()); _repeat202++) {
				if (contador < slist.size()) {
					if (slist.get((int)(contador)).equals("DÃ­a")) {
						contador++;
					}
					else {
		...
		//fin de carga
		//comprobar si el fichero de configuraciÃ³n establece carga automÃ¡tica 
		if (FileUtil.isExistFile(transferencias.getString("Ruta", "").concat("/configuraciÃ³n.txt"))) {
			slist.clear();
			str = FileUtil.readFile(transferencias.getString("Ruta", "").concat("/configuraciÃ³n.txt"));
			String [] arr = str.split("\n");
			for (int i = 0; i < arr.length; i++) {
				slist.add(arr[i]);
			}
			...
			for(int _repeat227 = 0; _repeat227 < (int)(slist.size()); _repeat227++) {
				if (contador < slist.size()) {
					if (slist.get((int)(contador)).equals("(Cargar automÃ¡ticamente)")) {
						contador++;
			...
			textocargar.setText("Cargado. No se encuentra fichero de configuraciÃ³n (".concat(transferencias.getString("Ruta", "").concat("/configuraciÃ³n.txt) ".concat(""))));
			registro.add("Cargado. No se encuentra fichero de configuraciÃ³n (".concat(transferencias.getString("Ruta", "").concat("/configuraciÃ³n.txt) ".concat(""))));
		}
		_actualizarlistanotas();
		transferencias.edit().putString("Mostrar opciÃ³n", "Organizar").commit();
		_mostraropcion();
		...
		registro.add("".concat("".concat("".concat(". Fin de mÃ³dulo carga de pantalla. ".concat("".concat(""))))));
		_actualizarregistro();
		_cargarlistas();
	}
			...
			if (listaavisosmomentospersonalizados.size() > 0) {
				if (listaavisosformaspersonalizadas.size() > 0) {
					if (listaavisosfiltrospersonalizados.size() > 0) {
						if (listaavisosaudiopersonalizados.size() > 0) {
							if (separadoravisos.getText().toString().equals("No")) {
								separadoravisos.setText("Ãšltimos criterios del aviso\n\n".concat("".concat("")));
							}
							else {
								separadoravisos.setText(separadoravisos.getText().toString().concat("\n\nÃšltimos criterios del aviso. Eventos: ".concat(String.valueOf((long)(listaavisoseventospersonalizados.size())).concat("".concat("".concat("\n\n".concat("".concat("".concat("")))))))));
	...
	public void _cargarvariables() {
		...
		transferencias.edit().putString("Ãšltimo criterio de aviso", "No").commit();
		transferencias.edit().putString("Ãšltimo criterio", "No").commit();
		...
		transferencias.edit().putString("Carga rÃ¡pida a partir de datos", "0").commit();
		transferencias.edit().putString("Rutina", "Fin").commit();
		transferencias.edit().putString("PosiciÃ³n carga rÃ¡pida de datos", "0").commit();
		...
		_cargarpantalla();
		//Comprobar si el archivo de configuraciÃ³n requiere cargarlo automÃ¡ticamente 
		if (FileUtil.isExistFile(transferencias.getString("Ruta configuraciÃ³n", ""))) {
			slist.clear();
			str = FileUtil.readFile(transferencias.getString("Ruta configuraciÃ³n", ""));
			String [] arr = str.split("\n");
			...
			if (slist.size() > 0) {
				...
				for(int _repeat42 = 0; _repeat42 < (int)(slist.size()); _repeat42++) {
					if (Double.parseDouble(transferencias.getString("Contador", "")) < slist.size()) {
						if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador", "")))).equals("Cargar configuraciÃ³n automÃ¡ticamente")) {
							transferencias.edit().putString("Contador", String.valueOf((long)(Double.parseDouble(transferencias.getString("Contador", "")) + 1))).commit();
							if (slist.get((int)(Double.parseDouble(transferencias.getString("Contador", "")))).equals("Si")) {
								transferencias.edit().putString("Cargando configuraciÃ³n", "Si").commit();
								_cargarconfiguracion();
							}
							else {
...
// Verificar que la ruta del archivo no estÃ© vacÃ­a
if (!preferencias.getString("ruta archivo", "").isEmpty()) {
    try {
        // AÃ±adir los datos al final del archivo
    ...
    // Manejar el caso en que la ruta estÃ© vacÃ­a
    textview3.setText("La ruta del archivo no estÃ¡ especificada en las preferencias.");
}
		...
		if (transferencias.getString("Guardando registro", "").equals("No")) {
			...
			if (transferencias.getString("Guardar registro automÃ¡ticamente", "").equals("Si") || transferencias.getString("Registrar en archivo", "").equals("Si")) {
				if (FileUtil.isFile(transferencias.getString("Ruta registros", "").concat("/registro general.txt"))) {
					...
					// aÃ±adir dato a archivo de registro m. Verificar que la ruta del archivo no estÃ© vacÃ­a
...
*/
	...
	public void _cargarlistas() {
		...
		listasecciones.add("Cuadro de ediciÃ³n");
		listasecciones.add("Lista de notas");
		...
		listatipoconfiguracion.add("PosiciÃ³n");
		...
		dimensiones.setVisibility(View.GONE);
		transferencias.edit().putString("MÃ©todo2", "1").commit();
		if (transferencias.getString("MÃ©todo2", "").equals("2")) {
			listaorden.clear();
		...
		listatipocita.add("DÃ­as del mes");
		listatipocita.add("DÃ­as de la semana");
		...
		objetivos.clear();
		objetivos.add("Seleccione funciÃ³n");
		objetivos.add("Aviso");
	...
	public void _filtroaviso1() {
		...
		if (transferencias.getString("Ãšltimo criterio", "").equals("No")) {
			transferencias.edit().putString("Aviso correcto", "No").commit();
		}
		else {
		...
		if (transferencias.getString("Aviso correcto", "").equals("Si")) {
			transferencias.edit().putString("AÃ±adir", "Si").commit();
			if (transferencias.getString("Ãšltimo criterio", "").equals("Filtro")) {
				if (transferencias.getString("Aviso filtro", "").equals("Seleccione filtro/s de aviso")) {
					transferencias.edit().putString("AÃ±adir", "Si").commit();
				}
				else {
			...
			if (transferencias.getString("Ãšltimo criterio", "").equals("Momento")) {
				if (transferencias.getString("Aviso momento", "").equals("Seleccione momento/s de aviso")) {
					transferencias.edit().putString("AÃ±adir", "No").commit();
				}
				else {
			...
			if (transferencias.getString("Ãšltimo criterio", "").equals("Forma")) {
				if (transferencias.getString("Aviso forma", "").equals("Seleccione forma/s de aviso")) {
					transferencias.edit().putString("AÃ±adir", "No").commit();
				}
				else {
			...
			if (transferencias.getString("Ãšltimo criterio", "").equals("Evento")) {
				if (transferencias.getString("Aviso evento", "").equals("Seleccione tipo de evento/s")) {
					transferencias.edit().putString("AÃ±adir", "No").commit();
				}
				else {
			...
			if (transferencias.getString("Ãšltimo criterio", "").equals("Aviso audio")) {
				transferencias.edit().putString("AÃ±adir", "Si").commit();
			}
			else {
				
			}
			if (transferencias.getString("AÃ±adir", "").equals("Si")) {
				if (transferencias.getString("Ãšltimo criterio", "").equals("Evento")) {
					transferencias.edit().putString("Estado evento", "Terminado").commit();
					avisoseventostemporal.clear();
				...
				if (transferencias.getString("Ãšltimo criterio", "").equals("Momento")) {
					...
					transferencias.edit().putString("Estado momento", "Terminado").commit();
					transferencias.edit().putString("Registro", transferencias.getString("Hora completa", "").concat(" - ".concat(transferencias.getString("Fecha completa", "").concat("AÃ±adir momento. Eventos: ".concat(String.valueOf((long)(avisoseventostemporal.size())).concat(". Momentos: ".concat(String.valueOf((long)(avisosmomentostemporal.size())).concat(". Formas: ".concat(String.valueOf((long)(avisosformastemporal.size())).concat("".concat(". Aviso correcto: ".concat(transferencias.getString("Aviso correcto", "").concat(". AÃ±adir: ".concat(transferencias.getString("AÃ±adir", "").concat(". Evento: ".concat(transferencias.getString("Aviso evento", "").concat(". Momento: ".concat(transferencias.getString("Aviso momento", "").concat(". Forma: ".concat(transferencias.getString("Aviso forma", "").concat(". Ãšltimo criterio: ".concat(transferencias.getString("Ãšltimo criterio", "").concat(". AÃ±adir: ".concat(transferencias.getString("AÃ±adir", "").concat("".concat("")))))))))))))))))))))))))).commit();
				...
				//comprobar si hay avisos repetidos en los Ãºltimos elegidos y si alguno de los elegidos son iguales que los que existen. Falta comparar con los avisos establecidos y con el criterio audio 
				transferencias.edit().putString("Aviso repetido", "No").commit();
				//asegurar que todas las matrices tengan el mismo nÃºmero de datos 
	...
	public void _cargarconfiguracionpantalla() {
		if (FileUtil.isExistFile(transferencias.getString("Ruta configuraciÃ³n pantalla", ""))) {
			//cargar configuraciÃ³n
			textocargar.setText("Cargando configuraciÃ³n ");
			slist.clear();
			str = FileUtil.readFile(transferencias.getString("Ruta configuraciÃ³n pantalla", ""));
			String [] arr = str.split("\n");
			for (int i = 0; i < arr.length; i++) {
				slist.add(arr[i]);
			}
			...
			for(int _repeat22 = 0; _repeat22 < (int)(slist.size()); _repeat22++) {
				if (contador < slist.size()) {
					if (slist.get((int)(contador)).equals("(Ancho editar)")) {
						...
						registro.add("Ancho de cuadro de ediciÃ³n establecido: ".concat(slist.get((int)(contador)).concat("")));
					}
					else {
					...
					if (slist.get((int)(contador)).equals("(Lista de notas)")) {
						...
						transferencias.edit().putString("PosiciÃ³n vertical lista de notas", slist.get((int)(contador))).commit();
						registro.add("PosiciÃ³n vertical de lista de notas establecido: ".concat(slist.get((int)(contador)).concat("")));
					}
					else {
					...
					if (slist.get((int)(contador)).equals("(Alto editar)")) {
						...
						registro.add("Alto de cuadro de ediciÃ³n establecido: ".concat(slist.get((int)(contador)).concat("")));
					}
					else {
						
					}
					if (slist.get((int)(contador)).equals("(PosiciÃ³n barra de herramientas vertical)")) {
						contador++;
						registro.add("Ancho de cuadro de ediciÃ³n establecido: ".concat(slist.get((int)(contador)).concat("")));
						contadorverticalbarra = Double.parseDouble(slist.get((int)(contador)));
						transferencias.edit().putString("PosiciÃ³n barra de herramientas vertical", slist.get((int)(contador))).commit();
						registro.add("PosiciÃ³n barra de herramientas vertical establecido: ".concat(slist.get((int)(contador)).concat("")));
					}
					else {
						
					}
					if (slist.get((int)(contador)).equals("(PosiciÃ³n barra de herramientas horizontal)")) {
						contador++;
						registro.add("Ancho de cuadro de ediciÃ³n establecido: ".concat(slist.get((int)(contador)).concat("")));
						contadorhorizontalbarra = Double.parseDouble(slist.get((int)(contador)));
						transferencias.edit().putString("PosiciÃ³n barra de herramientas horizontal", slist.get((int)(contador))).commit();
					}
					else {
						
					}
					if (slist.get((int)(contador)).equals("(Mostrar progreso por debajo de lÃ­neas)")) {
						
					}
					else {
						
					}
					registro.add("Cargando configuraciÃ³n de pantalla . (".concat(String.valueOf((long)(contador)).concat("/".concat(String.valueOf((long)(slist.size())).concat(") ".concat(slist.get((int)(contador)).concat("".concat(""))))))));
					contador++;
				}
				else {
			...
			textocargar.setText("Cargado");
			SketchwareUtil.showMessage(getApplicationContext(), "ConfiguraciÃ³n de pantalla cargada");
			_actualizarregistro();
		}
		else {
			SketchwareUtil.showMessage(getApplicationContext(), "No existen datos de configuraciÃ³n de pantalla ");
		}
		transferencias.edit().putString("MÃ©todo", "1").commit();
		if (transferencias.getString("MÃ©todo", "").equals("1")) {
	...
	public void _guardarconfiguracionpantalla() {
		...
		transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("(Alto editar)\n".concat(String.valueOf((long)(altoeditar)).concat("\n")))).commit();
		transferencias.edit().putString("ConfiguraciÃ³n", transferencias.getString("ConfiguraciÃ³n", "").concat("(Lista de notas)\n".concat(transferencias.getString("PosiciÃ³n vertical lista de notas", "").concat("\n")))).commit();
		FileUtil.writeFile(transferencias.getString("Ruta configuraciÃ³n pantalla", ""), transferencias.getString("ConfiguraciÃ³n", ""));
		transferencias.edit().putString("ConfiguraciÃ³n cambiada", "No").commit();
		registro.add("Guardada configuraciÃ³n de pantalla en fichero (".concat(transferencias.getString("Ruta 1", "").concat("/configuraciÃ³n \n".concat(dato.concat("")))));
		_actualizarregistro();
		botonguardarconfiguracion.setVisibility(View.INVISIBLE);
		listview1.setBackgroundColor(Color.TRANSPARENT);
	}
